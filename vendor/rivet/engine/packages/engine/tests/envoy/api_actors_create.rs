...
fn create_actor_with_key() {
	common::run(
		...
		|ctx| async move {
			...
			// Verify actor exists
			let actor =
				common::assert_actor_exists(ctx.leader_dc().guard_port(), &actor_id, &namespace)
					.await;
...
#[test]
fn create_actor_sleep_crash_policy() {
	common::run(
		common::TestOpts::new(1).with_timeout(30),
		|ctx| async move {
			let (namespace, _, _runner) =
...
fn create_actor_remote_datacenter_verify() {
	common::run(
		...
		|ctx| async move {
			...
			let actor =
				common::assert_actor_exists(ctx.get_dc(2).guard_port(), &actor_id, &namespace)
					.await;
			common::assert_actor_in_dc(&actor.actor_id.to_string(), 2).await;
		},
	);
