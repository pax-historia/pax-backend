...
async fn test_rocksdb_driver() {
	...
	run_all_tests(db.clone()).await;
	clear_test_namespace(&db).await.unwrap();
	test_rocksdb_read_only_transactions_do_not_retry_on_stale_reads(&db).await;
	clear_test_namespace(&db).await.unwrap();
}
...
async fn test_rocksdb_read_only_transactions_do_not_retry_on_stale_reads(db: &Database) {
	use std::sync::{
		Arc,
	...
	let key = test_subspace.pack(&("read_only_no_retry",));
	...
	assert_eq!(value, Some(b"initial".to_vec().into()));
...
async fn test_plain_set_preserves_versionstamp_like_binary(db: &Database) {
	...
	let mut value = Vec::from([
		0x33, 0x5a, 0x33, 0x59, 0x41, 0x37, 0x6c, 0x1d, 0x00, 0x09, 0x1f, 0x6b, 0x1d, 0x00, 0x09,
		0x1f,
	]);
