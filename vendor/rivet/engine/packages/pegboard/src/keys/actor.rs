...
#[derive(Debug)]
pub struct TotalActorsMetricIncrementedKey {
	actor_id: Id,
}

impl TotalActorsMetricIncrementedKey {
	pub fn new(actor_id: Id) -> Self {
		TotalActorsMetricIncrementedKey { actor_id }
	}
}

impl FormalKey for TotalActorsMetricIncrementedKey {
	type Value = ();
...
impl TuplePack for TotalActorsMetricIncrementedKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
	) -> std::io::Result<VersionstampOffset> {
		let t = (ACTOR, DATA, self.actor_id, TOTAL_ACTORS_METRIC_INCREMENTED);
		t.pack(w, tuple_depth)
	}
}

impl<'de> TupleUnpack<'de> for TotalActorsMetricIncrementedKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (input, (_, _, actor_id, key_type)) =
			<(usize, usize, Id, usize)>::unpack(input, tuple_depth)?;

		if key_type != TOTAL_ACTORS_METRIC_INCREMENTED {
			return Err(PackError::Message(
				"expected TOTAL_ACTORS_METRIC_INCREMENTED key type".into(),
			));
		}

		let v = TotalActorsMetricIncrementedKey { actor_id };
...
#[derive(Debug)]
pub struct LaneHintKey {
	actor_id: Id,
}

impl LaneHintKey {
	pub fn new(actor_id: Id) -> Self {
		LaneHintKey { actor_id }
	}
}

impl FormalKey for LaneHintKey {
	type Value = String;
...
impl TuplePack for LaneHintKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
	) -> std::io::Result<VersionstampOffset> {
		let t = (ACTOR, DATA, self.actor_id, LANE_HINT);
		t.pack(w, tuple_depth)
	}
}

impl<'de> TupleUnpack<'de> for LaneHintKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (input, (_, _, actor_id, _)) = <(usize, usize, Id, usize)>::unpack(input, tuple_depth)?;

		let v = LaneHintKey { actor_id };
