...
use crate::actor::messages::{
	ActorEvent, BroadcastPressure, QueueSendResult, Request, Response, StateDelta,
};
...
impl RegistryDispatcher {
	...
	async fn apply_tunnel_pressure(
		&self,
		actor_id: &str,
		...
		let Some(instance) = instance else {
			tracing::debug!(
				...
				"ignoring tunnel pressure for unknown actor",
			);
			return;
		};

		instance.ctx.apply_broadcast_pressure(BroadcastPressure {
...
mod tests {
	...
	#[tokio::test]
	async fn tunnel_pressure_updates_active_actor_context() {
		let actor_id = "pressure-actor".to_owned();
		let actor_name = "pressure".to_owned();
		let ctx = ActorContext::new(&actor_id, &actor_name, Vec::new(), "local");
		let (event_tx, mut event_rx) = mpsc::unbounded_channel();
		ctx.configure_actor_events(Some(event_tx));

		let factory = Arc::new(ActorFactory::new(ActorConfig::default(), |_start| {
			Box::pin(async { Ok(()) })
		}));
		let (lifecycle_tx, _lifecycle_rx) = mpsc::unbounded_channel();
		let (dispatch_tx, _dispatch_rx) = mpsc::unbounded_channel();
		let instance = Arc::new(ActorTaskHandle {
			...
			lifecycle: lifecycle_tx,
			dispatch: dispatch_tx,
			join: Arc::new(TokioMutex::new(Some(tokio::spawn(async { Ok(()) })))),
		});
		...
		dispatcher
			.set_actor_instance_state(actor_id.clone(), ActorInstanceState::Active(instance))
			.await;

		dispatcher
			.apply_tunnel_pressure(
				&actor_id,
				protocol::ToEnvoyTunnelPressure {
					gateway_id: [1, 2, 3, 4],
					request_id: Some([5, 6, 7, 8]),
					pressure: protocol::Pressure {
						credit: 12,
						queue_depth: 3,
						oldest_age_ms: Some(44),
					},
				},
			)
			.await;

		let expected = BroadcastPressure {
			credit: 12,
			queue_depth: 3,
			oldest_age_ms: Some(44),
		};
		assert_eq!(ctx.broadcast_pressure(), expected);
		match event_rx
			.try_recv()
			.expect("pressure should emit an actor event")
		{
			ActorEvent::BroadcastPressure { pressure } => assert_eq!(pressure, expected),
