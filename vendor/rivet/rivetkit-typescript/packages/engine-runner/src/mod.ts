...
export interface TunnelPressureUpdate {
	pressure: protocol.Pressure | null;
	gateways: ReadonlyMap<string, protocol.Pressure>;
	requests?: ReadonlyMap<string, protocol.Pressure>;
}
...
export class Runner {
	...
	#pendingTunnelMessages: protocol.ToServerTunnelMessage[] = [];
	#pendingTunnelFlushScheduled: boolean = false;
	#nextTunnelEpochByGateway: Map<string, bigint> = new Map();
	#tunnelPressureByGateway: Map<string, protocol.Pressure> = new Map();
	#tunnelPressureByRequest: Map<string, protocol.Pressure> = new Map();
	#lastTunnelPressureSignature: string = "";
	...
	async #openPegboardWebSocket() {
		...
		ws.addEventListener("message", async (ev) => {
			...
			} else if (message.tag === "ToClientTickWave") {
				const wave = message.val.wave;
				this.#applyTunnelPressure(
					wave.gatewayId,
					null,
					wave.backpressure,
				);
				this.#tunnel?.handleTickWave(wave).catch((err) => {
					this.log?.error({
						msg: "error handling tunnel tick wave",
						error: stringifyError(err),
					});
				});
			} else if (message.tag === "ToClientTunnelControl") {
				this.#handleTunnelControl(message.val.control);
	...
	__sendToServer(message: protocol.ToServer) {
		if (message.tag === "ToServerTunnelMessage") {
			this.#enqueueTunnelMessage(message.val);
			return;
		}

		this.#flushTunnelMessages();
		this.#sendEncodedToServer(message);
	}

	#enqueueTunnelMessage(message: protocol.ToServerTunnelMessage) {
		this.#pendingTunnelMessages.push(message);

		this.#scheduleTunnelFlush();
	}

	#scheduleTunnelFlush() {
		if (!this.#pendingTunnelFlushScheduled) {
			this.#pendingTunnelFlushScheduled = true;
			queueMicrotask(() => {
				this.#flushTunnelMessages();
			});
		}
	...
	#applyTunnelPressure(
		...
		const pressureByScope = requestId
			? this.#tunnelPressureByRequest
			: this.#tunnelPressureByGateway;
		const pressureKey = requestId
			? requestPressureKey(gatewayId, requestId)
			: gatewayKey;
		if (pressure) {
			pressureByScope.set(pressureKey, pressure);
			this.log?.debug({
				msg: "received tunnel pressure",
				gatewayId: gatewayKey,
				requestId: requestId ? idToStr(requestId) : null,
				pressure,
			});

			if (pressure.credit > 0) {
				this.#scheduleTunnelFlush();
			}
		} else if (pressureByScope.delete(pressureKey)) {
			this.#scheduleTunnelFlush();
		}
		this.#notifyTunnelPressure();
	}

	#handleTunnelControl(control: protocol.TunnelControl) {
		const pressure = pressureFromTunnelControl(control);
		if (pressure) {
			this.#applyTunnelPressure(
				pressure.gatewayId,
				pressure.requestId,
		...
		this.log?.debug({
			msg: "received tunnel control",
			control,
		});
	}

	#flushTunnelMessages() {
		if (this.#pendingTunnelMessages.length === 0) {
			this.#pendingTunnelFlushScheduled = false;
			return;
		}

		const messages = this.#pendingTunnelMessages;
		this.#pendingTunnelMessages = [];
		this.#pendingTunnelFlushScheduled = false;
		const { ready, blocked } = partitionToServerTunnelMessagesByPressure(
			messages,
			this.#tunnelPressureByGateway,
			this.#tunnelPressureByRequest,
		);
		this.#pendingTunnelMessages = blocked;
		this.#notifyTunnelPressure();

		if (blocked.length > 0) {
			this.log?.debug({
				msg: "paused tunnel messages due to gateway pressure",
		...
		for (const tickWave of buildToServerTickWaves(
			ready,
			this.#nextTunnelEpochByGateway,
		)) {
			this.#sendEncodedToServer({
				tag: "ToServerTickWave",
				val: tickWave,
			});
		}
	}

	#notifyTunnelPressure() {
		const gatewayEntries = [...this.#tunnelPressureByGateway.entries()].sort(
			([a], [b]) => a.localeCompare(b),
		);
		const requestEntries = [...this.#tunnelPressureByRequest.entries()].sort(
			([a], [b]) => a.localeCompare(b),
		);
		const signature = [
			...gatewayEntries.map((entry) => ["gateway", ...entry] as const),
			...requestEntries.map((entry) => ["request", ...entry] as const),
		]
			.map(
				([scope, pressureKey, pressure]) =>
					`${scope}:${pressureKey}:${pressure.credit}:${pressure.queueDepth}:${pressure.oldestAgeMs?.toString() ?? ""}`,
			)
			.join("|");
		if (signature === this.#lastTunnelPressureSignature) {
			return;
		}

		this.#lastTunnelPressureSignature = signature;
		this.#config.onTunnelPressure?.({
			pressure: aggregateGatewayPressures([
				...gatewayEntries.map(([, pressure]) => pressure),
				...requestEntries.map(([, pressure]) => pressure),
			]),
			gateways: new Map(gatewayEntries),
			requests: new Map(requestEntries),
		});
	}

	#sendEncodedToServer(message: protocol.ToServer) {
		this.log?.debug({
