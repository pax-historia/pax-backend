...
impl ProxyState {
	...
	async fn resolve_route(
		...
		let res = if let Some(res) = cache_res {
			...
			let prev_route_refresh = req_ctx.is_route_refresh();
			req_ctx.set_route_refresh(ignore_cache);
			let routing_res = timeout(route_timeout, (self.routing_fn)(req_ctx)).await;
			req_ctx.set_route_refresh(prev_route_refresh);
			let routing_res = routing_res.map_err(|_| {
...
mod tests {
	...
	use super::*;
	use crate::{RouteConfig, RoutingOutput};

	#[tokio::test]
	async fn resolve_route_marks_ignore_cache_calls_as_route_refresh() {
		let observed_route_refresh = Arc::new(Mutex::new(Vec::new()));
		let routing_observations = observed_route_refresh.clone();
		let routing_fn: RoutingFn = Arc::new(move |req_ctx| {
			let routing_observations = routing_observations.clone();
			Box::pin(async move {
				routing_observations
					.lock()
					.unwrap()
					.push(req_ctx.is_route_refresh());
		...
		let cache_key_fn: CacheKeyFn = Arc::new(|_req_ctx| Ok(0));
		let config = rivet_config::Config::from_root(rivet_config::config::Root::default());
		let state = ProxyState::new(config, routing_fn, cache_key_fn);
		let mut req_ctx = RequestContext::new(
		...
		state.resolve_route(&mut req_ctx, false).await.unwrap();
		state.resolve_route(&mut req_ctx, true).await.unwrap();

		assert_eq!(*observed_route_refresh.lock().unwrap(), vec![false, true]);
		assert!(!req_ctx.is_route_refresh());
	}
}
