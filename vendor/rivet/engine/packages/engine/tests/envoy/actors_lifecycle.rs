...
fn routing_directory_ready_envoy_total() -> u64 {
	routing_directory_lookup_total("ready", "envoy")
}
...
#[test]
fn envoy_guard_hot_connects_use_routing_directory() {
	common::run(
		common::TestOpts::new(1).with_timeout(30),
		|ctx| async move {
			let (namespace, _) = common::setup_test_namespace(ctx.leader_dc()).await;
			...
			let ready_before = routing_directory_ready_envoy_total();
			tokio::time::timeout(std::time::Duration::from_secs(5), async {
				loop {
					let response = common::ping_actor_via_guard(ctx.leader_dc(), &actor_id).await;
					assert_eq!(response["actorId"], actor_id);
					...
					if routing_directory_ready_envoy_total() > ready_before {
						break;
					}
			...
			.await
			.expect("guard route path should observe a ready envoy routing-directory entry");

			let warmed_before = routing_directory_ready_envoy_total();
			for _ in 0..8 {
				let response = common::ping_actor_via_guard(ctx.leader_dc(), &actor_id).await;
				assert_eq!(response["actorId"], actor_id);
				assert_eq!(response["status"], "ok");
			...
			assert!(
				routing_directory_ready_envoy_total() >= warmed_before + 8,
				"hot guard HTTP connects should continue hitting the envoy routing directory"
			);
		},
	);
}
...
#[test]
fn envoy_guard_stale_directory_entry_falls_back_and_rewarms() {
	common::run(
		common::TestOpts::new(1).with_timeout(30),
		|ctx| async move {
			...
			.await
			.expect("guard route path should warm the envoy routing-directory entry");
			...
			pegboard::routing_directory::publish_delta(
				...
				pegboard::routing_directory::RoutingDelta::Stale {
					...
					generation: 1,
					target: Some(pegboard::routing_directory::RoutingTarget::Envoy {
						namespace_id,
			...
			let stale_before = routing_directory_lookup_total("stale", "envoy");
			tokio::time::timeout(std::time::Duration::from_secs(5), async {
				loop {
					let response = common::ping_actor_via_guard(ctx.leader_dc(), &actor_id).await;
					assert_eq!(response["actorId"], actor_id);
					...
					if routing_directory_lookup_total("stale", "envoy") > stale_before {
						break;
					}
			...
			.await
			.expect("stale envoy directory entry should fall back through storage");

			let rewarmed_before = routing_directory_ready_envoy_total();
			tokio::time::timeout(std::time::Duration::from_secs(5), async {
				loop {
					let response = common::ping_actor_via_guard(ctx.leader_dc(), &actor_id).await;
					assert_eq!(response["actorId"], actor_id);
					...
					if routing_directory_ready_envoy_total() > rewarmed_before {
						break;
					}
			...
			.await
			.expect("storage fallback should rewarm the envoy routing-directory entry");
		},
	);
}

#[test]
fn envoy_guard_wrong_directory_target_refreshes_to_storage_route() {
	common::run(
		...
		|ctx| async move {
			...
			pegboard::routing_directory::publish_delta(
				...
				pegboard::routing_directory::RoutingDelta::Ready {
					actor_id: parsed_actor_id,
					generation: 1,
					target: pegboard::routing_directory::RoutingTarget::Envoy {
						namespace_id,
						envoy_key: format!("dead-{}", envoy.envoy_key),
					},
				},
			)
			.await
			...
			.await
			.expect("wrong envoy directory target should route-refresh through storage");
			...
			.await
			.expect("route refresh should rewarm the storage-backed envoy directory entry");
		},
	);
}

#[test]
fn envoy_guard_websocket_wrong_directory_target_refreshes_to_storage_route() {
	common::run(
		common::TestOpts::new(1)
			.with_timeout(30)
			.with_gateway_websocket_open_timeout_ms(100),
		|ctx| async move {
			let (namespace, namespace_id) = common::setup_test_namespace(ctx.leader_dc()).await;
			...
			assert!(
				...
				"wrong envoy directory target should route-refresh through storage"
			);
			...
			.await
			.expect("websocket route refresh should rewarm the storage-backed envoy entry");
		},
	);
}
...
fn envoy_http_tunnel_round_trips_request_and_errors() {
	common::run(
		...
		|ctx| async move {
			...
			let large_payload: serde_json::Value = large_response
				.json()
				.await
				.expect("invalid large echo response");
...
fn query_get_or_create_from_dc_without_runner_forwards_to_runner_dc() {
	common::run(
		common::TestOpts::new(2).with_timeout(45),
		|ctx| async move {
...
fn envoy_crash_policy_sleep() {
	common::run(
		common::TestOpts::new(1).with_timeout(75),
		|ctx| async move {
			let (namespace, _) = common::setup_test_namespace(ctx.leader_dc()).await;
...
fn envoy_actor_pending_allocation_no_envoys() {
	common::run(common::TestOpts::new(1), |ctx| async move {
		...
		assert!(
			matches!(&actor.error, Some(rivet_types::actor::ActorError::NoEnvoys)),
