...
const SLOW_GATEWAY_WEBSOCKET_OPEN_ACK_MS: u128 = 1000;

fn log_slow_gateway_websocket_open_ack(
	actor_id: Id,
	runner_id: Id,
	request_id: protocol::mk2::RequestId,
	publish_duration: Duration,
	runner_wait_duration: Duration,
) {
	let gateway_websocket_open_publish_duration_ms = publish_duration.as_millis();
	let gateway_websocket_open_runner_wait_duration_ms = runner_wait_duration.as_millis();
	let gateway_websocket_open_ack_duration_ms =
		gateway_websocket_open_publish_duration_ms + gateway_websocket_open_runner_wait_duration_ms;

	if gateway_websocket_open_ack_duration_ms >= SLOW_GATEWAY_WEBSOCKET_OPEN_ACK_MS {
		tracing::warn!(
			?actor_id,
			?runner_id,
			request_id=%protocol::util::id_to_string(&request_id),
			gateway_websocket_open_ack_duration_ms,
			gateway_websocket_open_publish_duration_ms,
			gateway_websocket_open_runner_wait_duration_ms,
			"slow gateway websocket open ack"
		);
	}
}
...
impl PegboardGateway {
	...
	async fn handle_websocket_inner(
		...
		let can_hibernate = if after_hibernation {
			...
			let open_publish_started = Instant::now();
			self.shared_state
				.send_message(request_id, open_message)
				.await?;
			let open_publish_duration = open_publish_started.elapsed();
			...
			log_slow_gateway_websocket_open_ack(
				self.actor_id,
				self.runner_id,
				request_id,
				open_publish_duration,
				open_wait_started.elapsed(),
			);
		...
		.run(|tx| async move {
			...
			let Some(namespace_id) = namespace_id else {
				tracing::debug!(
					?runner_id,
					?actor_id,
					"skipping request metrics for unknown runner"
				);
				return Ok((protocol_version, true, actor_workflow_id));
			};
			...
			Ok((protocol_version, has_name, actor_workflow_id))
		})
		.await?;
