...
#[test]
fn broadcast_pressure_updates_budget_and_emits_event_once() {
	let ctx = ActorContext::new("pressure-actor", "actor", Vec::new(), "local");
	assert_eq!(ctx.broadcast_pressure(), BroadcastPressure::default());

	let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
	ctx.configure_actor_events(Some(tx));

	let pressure = BroadcastPressure {
		credit: 0,
		queue_depth: 9,
		oldest_age_ms: Some(250),
	};
	ctx.apply_broadcast_pressure(pressure);

	assert_eq!(ctx.broadcast_pressure(), pressure);
	match rx.try_recv().expect("pressure event should be emitted") {
		ActorEvent::BroadcastPressure { pressure: emitted } => assert_eq!(emitted, pressure),
		other => panic!("expected broadcast pressure event, got {}", other.kind()),
	}

	ctx.apply_broadcast_pressure(pressure);
	assert!(rx.try_recv().is_err(), "duplicate pressure should not emit");
}
...
mod moved_tests {
	...
	fn fresh_last_ping_ts() -> std::sync::atomic::AtomicI64 {
	...
	fn build_envoy_handle_with_live_connections(
		...
		let shared = Arc::new(SharedContext {
			...
			last_ping_ts: fresh_last_ping_ts(),
	...
	#[test]
	fn broadcast_fans_out_shared_event_to_subscribed_connections() {
		let ctx = ActorContext::new("broadcast-actor", "actor", Vec::new(), "local");
		let sent = Arc::new(Mutex::new(Vec::new()));
		let encode_count = Arc::new(AtomicUsize::new(0));

		let subscribed_a = ConnHandle::new("subscribed-a", Vec::new(), Vec::new(), false);
		let subscribed_b = ConnHandle::new("subscribed-b", Vec::new(), Vec::new(), false);
		let idle = ConnHandle::new("idle", Vec::new(), Vec::new(), false);

		for conn in [&subscribed_a, &subscribed_b] {
			conn.subscribe("updated");
			let sent = sent.clone();
			conn.configure_event_sender(Some(Arc::new(move |event| {
				sent.lock().expect("sent events lock poisoned").push(event);
				Ok(())
			})));
		}
		idle.configure_event_sender(Some(Arc::new(|_| {
			panic!("unsubscribed connection should not receive broadcast")
		})));

		ctx.insert_existing(subscribed_a);
		ctx.insert_existing(subscribed_b);
		ctx.insert_existing(idle);

		ctx.broadcast("updated", b"payload");

		let sent = sent.lock().expect("sent events lock poisoned");
		assert_eq!(sent.len(), 2);
		for event in sent.iter() {
			assert_eq!(event.name(), "updated");
			assert_eq!(event.args(), b"payload");
		}

		let encode_count_for_first = encode_count.clone();
		let first_payload = sent[0]
			.cached_payload("test:broadcast-payload", move |name, args| {
				encode_count_for_first.fetch_add(1, Ordering::SeqCst);
				Ok([name.as_bytes(), b":", args].concat())
			})
			.expect("first payload encode should succeed");
		let encode_count_for_second = encode_count.clone();
		let second_payload = sent[1]
			.cached_payload("test:broadcast-payload", move |_name, _args| {
				encode_count_for_second.fetch_add(1, Ordering::SeqCst);
				Ok(b"should-not-reencode".to_vec())
			})
			.expect("second payload lookup should use cache");

		assert_eq!(first_payload.as_ref(), b"updated:payload");
		assert_eq!(second_payload.as_ref(), b"updated:payload");
