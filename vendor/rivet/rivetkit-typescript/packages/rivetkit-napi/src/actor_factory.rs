...
use rivetkit_core::{
	...
	ActorFactory as CoreActorFactory, BroadcastPressure, ConnHandle as CoreConnHandle, Request,
...
#[derive(Clone)]
pub(crate) struct BroadcastPressurePayload {
	pub(crate) ctx: CoreActorContext,
	pub(crate) pressure: BroadcastPressure,
}
...
pub(crate) struct CallbackBindings {
	...
	pub(crate) on_broadcast_pressure: Option<CallbackTsfn<BroadcastPressurePayload>>,
}
...
impl CallbackBindings {
	fn from_js(callbacks: JsObject) -> napi::Result<Self> {
		...
		Ok(Self {
			...
			on_broadcast_pressure: optional_tsfn(
				&callbacks,
				"onBroadcastPressure",
				build_broadcast_pressure_payload,
			)?,
		})
	}
}
...
impl TsfnPayloadSummary for BroadcastPressurePayload {
	fn payload_summary(&self) -> String {
		format!(
			"actor_id={} credit={} queue_depth={} oldest_age_ms={:?}",
			self.ctx.actor_id(),
			self.pressure.credit,
			self.pressure.queue_depth,
			self.pressure.oldest_age_ms,
		)
	}
}
...
fn build_broadcast_pressure_payload(
	env: &Env,
	payload: BroadcastPressurePayload,
) -> napi::Result<Vec<napi::JsUnknown>> {
	let mut pressure = env.create_object()?;
	pressure.set("credit", payload.pressure.credit)?;
	pressure.set("queueDepth", payload.pressure.queue_depth)?;
	match payload.pressure.oldest_age_ms {
		Some(oldest_age_ms) => pressure.set("oldestAgeMs", oldest_age_ms)?,
		None => pressure.set("oldestAgeMs", env.get_null()?)?,
	}

	let mut object = env.create_object()?;
	object.set("ctx", ActorContext::new(payload.ctx))?;
	object.set("pressure", pressure)?;
	Ok(vec![object.into_unknown()])
}
