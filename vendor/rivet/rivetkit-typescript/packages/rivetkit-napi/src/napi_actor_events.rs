...
use crate::actor_factory::{
	...
	BeforeSubscribePayload, BroadcastPressurePayload, CallbackBindings, ConnectionPayload,
	CreateConnStatePayload, CreateStatePayload, HttpRequestPayload, LifecyclePayload,
	MigratePayload, QueueSendPayload, SerializeStatePayload, WebSocketPayload,
	WorkflowHistoryPayload, WorkflowReplayPayload, call_buffer, call_optional_buffer,
	call_queue_send, call_request, call_state_delta_payload, call_void,
};
...
pub(crate) async fn dispatch_event(
	...
	match event {
		...
		ActorEvent::BroadcastPressure { pressure } => {
			let Some(callback) = bindings.on_broadcast_pressure.clone() else {
				return;
			};
			let ctx = ctx.clone();
			tasks.spawn(async move {
				if let Err(error) = call_broadcast_pressure(&callback, &ctx, pressure).await {
					tracing::error!(
						actor_id = %ctx.inner().actor_id(),
						?error,
						"broadcast pressure callback failed",
					);
				}
			});
		}
...
async fn call_broadcast_pressure(
	callback: &crate::actor_factory::CallbackTsfn<BroadcastPressurePayload>,
	ctx: &ActorContext,
	pressure: rivetkit_core::BroadcastPressure,
) -> Result<()> {
	call_void(
		"onBroadcastPressure",
		callback,
		BroadcastPressurePayload {
			ctx: ctx.inner().clone(),
			pressure,
		},
	)
