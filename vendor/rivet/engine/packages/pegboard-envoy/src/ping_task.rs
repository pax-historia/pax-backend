...
pub async fn task(
	...
	loop {
		...
		// Keep routing-directory liveness tied to the live socket, not the DB ping write.
		publish_routing_heartbeat(&ctx, &conn).await;
...
async fn publish_routing_heartbeat(ctx: &StandaloneCtx, conn: &Conn) {
	let Ok(ups) = ctx.ups() else {
		tracing::warn!(
			namespace_id=?conn.namespace_id,
			envoy_key=%conn.envoy_key,
			"failed to get ups for envoy routing heartbeat"
		);
		return;
	};

	pegboard::routing_directory::publish_delta_best_effort(
		&ups,
