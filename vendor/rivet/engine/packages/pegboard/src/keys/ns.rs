...
impl FormalKey for RunnerAllocIdxKey {
	...
	fn serialize(&self, value: Self::Value) -> Result<Vec<u8>> {
		rivet_data::versioned::RunnerAllocIdxKeyData::wrap_latest(value)
...
#[derive(Debug)]
pub struct RunnerLaneAllocIdxKey {
	pub namespace_id: Id,
	pub name: String,
	pub lane: String,
	pub version: u32,
...
impl RunnerLaneAllocIdxKey {
	...
	pub fn subspace(namespace_id: Id, name: String, lane: String) -> RunnerLaneAllocIdxSubspaceKey {
		RunnerLaneAllocIdxSubspaceKey::new(namespace_id, name, Some(lane))
	}

	pub fn name_subspace(namespace_id: Id, name: String) -> RunnerLaneAllocIdxSubspaceKey {
		RunnerLaneAllocIdxSubspaceKey::new(namespace_id, name, None)
	}

	pub fn entire_subspace() -> RunnerLaneAllocIdxSubspaceKey {
		RunnerLaneAllocIdxSubspaceKey::entire()
	}
}

impl FormalKey for RunnerLaneAllocIdxKey {
	type Value = rivet_data::converted::RunnerAllocIdxKeyData;
...
impl TuplePack for RunnerLaneAllocIdxKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
...
impl<'de> TupleUnpack<'de> for RunnerLaneAllocIdxKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (
			input,
			(
		...
		) = <(usize, usize, Id, String, String, i32, i32, i64, Id)>::unpack(input, tuple_depth)?;

		let v = RunnerLaneAllocIdxKey {
			namespace_id,
			name,
			lane,
			version: -version as u32,
...
pub struct RunnerLaneAllocIdxSubspaceKey {
	pub namespace_id: Option<Id>,
	pub name: Option<String>,
	pub lane: Option<String>,
}

impl RunnerLaneAllocIdxSubspaceKey {
	pub fn new(namespace_id: Id, name: String, lane: Option<String>) -> Self {
		RunnerLaneAllocIdxSubspaceKey {
			namespace_id: Some(namespace_id),
			name: Some(name),
			lane,
		}
...
impl TuplePack for RunnerLaneAllocIdxSubspaceKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
		...
		let t = (NAMESPACE, RUNNER_LANE_ALLOC_IDX);
		offset += t.pack(w, tuple_depth)?;
		...
		if let Some(namespace_id) = &self.namespace_id {
			...
			if let Some(name) = &self.name {
				...
				if let Some(lane) = &self.lane {
					offset += lane.pack(w, tuple_depth)?;
				}
			}
		}
...
#[derive(Debug)]
pub struct RunnerAllocBucketIdxKey {
	pub namespace_id: Id,
	pub name: String,
	pub bucket: u16,
	pub version: u32,
...
impl RunnerAllocBucketIdxKey {
	...
	pub fn subspace(
		...
		name: String,
		bucket: u16,
	) -> RunnerAllocBucketIdxSubspaceKey {
		RunnerAllocBucketIdxSubspaceKey::new(namespace_id, name, bucket)
	}
}

impl FormalKey for RunnerAllocBucketIdxKey {
	type Value = rivet_data::converted::RunnerAllocIdxKeyData;
...
impl TuplePack for RunnerAllocBucketIdxKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
...
impl<'de> TupleUnpack<'de> for RunnerAllocBucketIdxKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (
			input,
			(
		...
		) = <(usize, usize, Id, String, u16, i32, i32, i64, Id)>::unpack(input, tuple_depth)?;

		let v = RunnerAllocBucketIdxKey {
			namespace_id,
			name,
			bucket,
			version: -version as u32,
...
pub struct RunnerAllocBucketIdxSubspaceKey {
	pub namespace_id: Id,
	pub name: String,
	pub bucket: u16,
}

impl RunnerAllocBucketIdxSubspaceKey {
	pub fn new(namespace_id: Id, name: String, bucket: u16) -> Self {
		RunnerAllocBucketIdxSubspaceKey {
			namespace_id,
			name,
			bucket,
...
impl TuplePack for RunnerAllocBucketIdxSubspaceKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
...
#[derive(Debug)]
pub struct RunnerLaneAllocBucketIdxKey {
	pub namespace_id: Id,
	pub name: String,
	pub lane: String,
	pub bucket: u16,
...
impl RunnerLaneAllocBucketIdxKey {
	pub fn new(
		namespace_id: Id,
		name: String,
		lane: String,
	...
	pub fn subspace(
		...
		lane: String,
		bucket: u16,
	) -> RunnerLaneAllocBucketIdxSubspaceKey {
		RunnerLaneAllocBucketIdxSubspaceKey::new(namespace_id, name, lane, bucket)
	}
}

impl FormalKey for RunnerLaneAllocBucketIdxKey {
	type Value = rivet_data::converted::RunnerAllocIdxKeyData;
...
impl TuplePack for RunnerLaneAllocBucketIdxKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
...
impl<'de> TupleUnpack<'de> for RunnerLaneAllocBucketIdxKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (
			input,
			(
		...
		) = <(usize, usize, Id, String, String, u16, i32, i32, i64, Id)>::unpack(input, tuple_depth)?;

		let v = RunnerLaneAllocBucketIdxKey {
			namespace_id,
			name,
			lane,
			bucket,
...
pub struct RunnerLaneAllocBucketIdxSubspaceKey {
	pub namespace_id: Id,
	pub name: String,
	pub lane: String,
	pub bucket: u16,
...
impl RunnerLaneAllocBucketIdxSubspaceKey {
	pub fn new(namespace_id: Id, name: String, lane: String, bucket: u16) -> Self {
		RunnerLaneAllocBucketIdxSubspaceKey {
			namespace_id,
			name,
			lane,
...
impl TuplePack for RunnerLaneAllocBucketIdxSubspaceKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
...
#[derive(Debug)]
pub struct PendingActorByRunnerNameSelectorAndLaneKey {
	pub namespace_id: Id,
	pub runner_name_selector: String,
	pub lane: String,
	pub ts: i64,
...
impl PendingActorByRunnerNameSelectorAndLaneKey {
	pub fn new(
		namespace_id: Id,
		runner_name_selector: String,
		lane: String,
		...
		PendingActorByRunnerNameSelectorAndLaneKey {
			namespace_id,
			runner_name_selector,
			lane,
			ts,
	...
	pub fn subspace(
		...
		runner_name_selector: String,
		lane: String,
	) -> PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
		PendingActorByRunnerNameSelectorAndLaneSubspaceKey::new(
			namespace_id,
			runner_name_selector,
			Some(lane),
		)
	...
	pub fn entire_subspace() -> PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
		PendingActorByRunnerNameSelectorAndLaneSubspaceKey::entire()
	}
}

impl FormalKey for PendingActorByRunnerNameSelectorAndLaneKey {
	/// Generation.
	type Value = u32;
...
impl TuplePack for PendingActorByRunnerNameSelectorAndLaneKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
		...
		let t = (
			NAMESPACE,
			PENDING_ACTOR_BY_RUNNER_NAME_SELECTOR_AND_LANE,
			self.namespace_id,
			&self.runner_name_selector,
			&self.lane,
			self.ts,
...
impl<'de> TupleUnpack<'de> for PendingActorByRunnerNameSelectorAndLaneKey {
	fn unpack(input: &[u8], tuple_depth: TupleDepth) -> PackResult<(&[u8], Self)> {
		let (input, (_, _, namespace_id, runner_name_selector, lane, ts, actor_id)) =
			<(usize, usize, Id, String, String, i64, Id)>::unpack(input, tuple_depth)?;

		let v = PendingActorByRunnerNameSelectorAndLaneKey {
			namespace_id,
			runner_name_selector,
			lane,
			ts,
...
pub struct PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
	pub namespace_id: Option<Id>,
	pub runner_name_selector: Option<String>,
	pub lane: Option<String>,
}

impl PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
	pub fn new(namespace_id: Id, runner_name_selector: String, lane: Option<String>) -> Self {
		PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
			namespace_id: Some(namespace_id),
			runner_name_selector: Some(runner_name_selector),
			lane,
...
impl TuplePack for PendingActorByRunnerNameSelectorAndLaneSubspaceKey {
	fn pack<W: std::io::Write>(
		&self,
		w: &mut W,
		tuple_depth: TupleDepth,
		...
		let t = (NAMESPACE, PENDING_ACTOR_BY_RUNNER_NAME_SELECTOR_AND_LANE);
		offset += t.pack(w, tuple_depth)?;
...
impl FormalKey for ActorByKeyKey {
	...
	fn deserialize(&self, raw: &[u8]) -> Result<Self::Value> {
		rivet_data::versioned::ActorByKeyKeyData::deserialize_with_embedded_version(raw)
	}

	fn serialize(&self, value: Self::Value) -> Result<Vec<u8>> {
		rivet_data::versioned::ActorByKeyKeyData::wrap_latest(value)
...
impl FormalKey for RunnerByKeyKey {
	...
	fn serialize(&self, value: Self::Value) -> Result<Vec<u8>> {
		rivet_data::versioned::RunnerByKeyKeyData::wrap_latest(value)
