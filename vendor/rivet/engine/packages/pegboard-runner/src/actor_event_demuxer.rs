...
use anyhow::Result;
use gas::prelude::*;
use pegboard::routing_directory::{RoutingDelta, RoutingTarget, publish_delta_best_effort};
...
async fn dispatch_events(
	...
	publish_runner_ready_delta(ctx, runner_id, actor_id, &events).await;
...
async fn publish_runner_ready_delta(
	ctx: &StandaloneCtx,
	runner_id: Id,
	actor_id: Id,
	events: &[protocol::mk2::EventWrapper],
) {
	let Some(generation) = runner_ready_generation(events) else {
		return;
	};
	...
	let ups = match ctx.ups() {
		Ok(ups) => ups,
		Err(err) => {
			tracing::warn!(?err, ?actor_id, "failed to get UPS for runner ready delta");
			return;
		}
	};
...
fn runner_ready_generation(events: &[protocol::mk2::EventWrapper]) -> Option<u64> {
	events.iter().rev().find_map(|event| match &event.inner {
		protocol::mk2::Event::EventActorStateUpdate(protocol::mk2::EventActorStateUpdate {
			state: protocol::mk2::ActorState::ActorStateRunning,
			..
		}) => Some(u64::from(event.checkpoint.generation)),
		_ => None,
	})
}
...
mod tests {
	...
	fn running_event(generation: u32, index: i64) -> protocol::mk2::EventWrapper {
		protocol::mk2::EventWrapper {
			checkpoint: protocol::mk2::ActorCheckpoint {
				actor_id: "actor".to_string(),
				generation,
				index,
			},
			inner: protocol::mk2::Event::EventActorStateUpdate(
	...
	#[test]
	fn ready_generation_uses_latest_running_event() {
		assert_eq!(
			Some(7),
			runner_ready_generation(&[running_event(3, 0), running_event(7, 1)])
		);
		assert_eq!(None, runner_ready_generation(&[]));
	}
}
