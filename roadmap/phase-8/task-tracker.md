...
| 2 | **Build matrix** — Run the local build matrix for active packages plus router and creator bundle artifacts; fix any compile/build break surfaced by the Phase 7 rewrite. | `to_do` |
| 3 | **Local Broker smoke** — Start the local stack with no production `.env`, run `pnpm smoke`, and fix runtime bugs until the Broker/Runner path opens a session, echoes over WS, writes checkpoint/history, and shuts down cleanly. | `to_do` |
