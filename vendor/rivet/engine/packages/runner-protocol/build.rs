...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	if cli_js_path.exists() {
		typescript::generate_sdk(repo_root, &schema_dir);
	} else {
		println!(
...
mod typescript {
	...
	pub fn generate_sdk(repo_root: &Path, schema_dir: &Path) {
		let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
		let _repo_root_from_manifest = Path::new(&manifest_dir)
			.parent()
			.and_then(|p| p.parent())
			.and_then(|p| p.parent())
