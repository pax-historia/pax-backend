...
pub fn stringify_to_envoy(message: &protocol::ToEnvoy) -> String {
	match message {
		...
		protocol::ToEnvoy::ToEnvoyTunnelPressure(val) => {
			format!(
				"ToEnvoyTunnelPressure{{gatewayId: {:?}, requestId: {:?}, credit: {}, queueDepth: {}, oldestAgeMs: {:?}}}",
				val.gateway_id,
				val.request_id,
				val.pressure.credit,
				val.pressure.queue_depth,
				val.pressure.oldest_age_ms
			)
		}
