...
| `scenario-suite.yml` | The adversarial release gate. Runs `testing/scenarios` plus the `historia-default` proof catalog across `ivm` and `noivm` with every nemesis profile; uploads histories/results/logs on failure. |
