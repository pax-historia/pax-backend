...
use super::*;
use crate::actor::connection::OutgoingEvent;
use crate::error::ProtocolError;

const ACTOR_CONNECT_EVENT_JSON_CACHE_KEY: &str = "actor-connect:event:json";
const ACTOR_CONNECT_EVENT_CBOR_CACHE_KEY: &str = "actor-connect:event:cbor";
const ACTOR_CONNECT_EVENT_BARE_CACHE_KEY: &str = "actor-connect:event:bare";
...
pub(super) fn send_actor_connect_event(
	sender: &WebSocketSender,
	encoding: ActorConnectEncoding,
	event: &OutgoingEvent,
	max_outgoing_message_size: usize,
	...
	match encoding {
		ActorConnectEncoding::Json => {
			let payload = event
				.cached_payload(ACTOR_CONNECT_EVENT_JSON_CACHE_KEY, |name, args| {
					encode_actor_connect_message_json(&ActorConnectToClient::Event(
						ActorConnectEvent {
							name: name.to_owned(),
							args: ByteBuf::from(args.to_vec()),
						},
					))
			...
			let payload = std::str::from_utf8(payload.as_ref())
				.expect("cached actor connect JSON event payload must be UTF-8");
			sender.send_text(payload);
		}
		ActorConnectEncoding::Cbor => {
			let payload = event
				.cached_payload(ACTOR_CONNECT_EVENT_CBOR_CACHE_KEY, |name, args| {
					encode_actor_connect_message_cbor(&ActorConnectToClient::Event(
						ActorConnectEvent {
							name: name.to_owned(),
							args: ByteBuf::from(args.to_vec()),
						},
			...
			sender.send(payload.to_vec(), true);
		}
		ActorConnectEncoding::Bare => {
			let payload = event
				.cached_payload(ACTOR_CONNECT_EVENT_BARE_CACHE_KEY, |name, args| {
					encode_actor_connect_message(&ActorConnectToClient::Event(ActorConnectEvent {
						name: name.to_owned(),
						args: ByteBuf::from(args.to_vec()),
					}))
				})
