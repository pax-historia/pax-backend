...
use crate::{
	executor::{
		...
		WorkerLanePlacementInput, actor_placement_key, place_actor_in_worker_lane,
		worker_lane_from_hint,
	},
	keys,
...
async fn insert_db(ctx: &ActivityCtx, input: &InsertDbInput) -> Result<()> {
	ctx.udb()?
		.run(|tx| async move {
			...
			let lane_key = keys::runner::LaneKey::new(input.runner_id);

			let (remaining_slots_entry, last_ping_ts_entry, old_lane_entry) = tokio::try_join!(
			...
			let old_worker_lane = worker_lane_from_hint(old_lane_entry.as_deref());
			tx.write(&lane_key, worker_lane.as_str().to_owned())?;
			...
			let legacy_alloc_key = keys::ns::RunnerAllocIdxKey::new(
				input.namespace_id,
				input.name.clone(),
				input.version,
			...
			crate::ops::runner::update_alloc_idx::clear_legacy_alloc_indexes(
				&tx,
				input.namespace_id,
				&input.name,
				input.runner_id,
			...
			.await?;
			crate::ops::runner::update_alloc_idx::clear_alloc_buckets(
				&tx,
				input.namespace_id,
				&input.name,
				&old_worker_lane,
			...
			.await?;
			if old_worker_lane != worker_lane {
				crate::ops::runner::update_alloc_idx::clear_alloc_buckets(
					&tx,
					input.namespace_id,
					&input.name,
			...
			if worker_lane == WorkerLane::default() {
				tx.write(&legacy_alloc_key, alloc_data)?;
			} else {
				tx.write(
			...
			crate::ops::runner::update_alloc_idx::write_distributed_alloc_buckets(
				&tx,
				input.namespace_id,
				&input.name,
				&worker_lane,
	...
	let allocations = ctx
		...
		.run(|tx| async move {
			...
			'queue_loop: loop {
				...
				let actor_name = tx
					.read(&keys::actor::NameKey::new(queue_key.actor_id), Snapshot)
					.await?;
				let actor_key = tx
					.read_opt(&keys::actor::KeyKey::new(queue_key.actor_id), Snapshot)
					.await?;
				let lane_hint = tx
					.read_opt(&keys::actor::LaneHintKey::new(queue_key.actor_id), Snapshot)
					.await?;
				...
				let worker_lane = worker_lane_from_hint(lane_hint.as_deref());
				...
				if worker_lane == WorkerLane::default() {
					...
					loop {
						...
						candidates.push(RunnerAllocIndexCandidate::from_default(
					...
					loop {
						...
						candidates.push(RunnerAllocIndexCandidate::from_lane(
							old_runner_alloc_key,
							old_runner_alloc_key_data,
						));
				...
				if let WorkerLanePlacement::Selected(assignment) = place_actor_in_worker_lane(
					...
					let Some(selected_candidate) = candidates
						...
						bail!("worker lane placement selected unknown pending runner");
					};
					...
					tx.delete(&queue_key);
					tx.delete(&keys::ns::PendingActorByRunnerNameSelectorAndLaneKey::new(
						input.namespace_id,
						input.name.clone(),
						worker_lane.as_str().to_owned(),
						queue_key.ts,
					...
					let new_remaining_millislots = assignment.remaining_millislots_after;
					...
					tx.write(
						&keys::runner::ActorKey::new(assignment.runner_id, queue_key.actor_id),
						generation,
					)?;
