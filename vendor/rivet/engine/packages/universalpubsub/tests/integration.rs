...
use universalpubsub::{
	LaneOrdering, LanePriority, LanePublishOpts, LaneSpec, NextOutput, OnFull, PubSub, PublishOpts,
	Subscriber,
};
use uuid::Uuid;
...
async fn test_inner(pubsub: &PubSub) {
	...
	let start = Instant::now();
	test_lane_local_fast_path_delivery(&pubsub).await.unwrap();
	tracing::info!(duration_ms = ?start.elapsed().as_millis(), "test_lane_local_fast_path_delivery completed");

	let start = Instant::now();
	test_lane_driver_broadcast_delivery(&pubsub).await.unwrap();
	tracing::info!(duration_ms = ?start.elapsed().as_millis(), "test_lane_driver_broadcast_delivery completed");
}
...
fn integration_lane_spec(local_fast_path: bool) -> LaneSpec {
	LaneSpec {
		capacity: 8,
...
async fn expect_lane_message(subscriber: &mut Subscriber, expected: &[u8]) -> Result<()> {
	match tokio::time::timeout(Duration::from_secs(5), subscriber.next()).await {
		Ok(Ok(NextOutput::Message(msg))) => assert_eq!(msg.payload, expected),
		Ok(Ok(NextOutput::Unsubscribed)) => panic!("unexpected unsubscribe"),
		Ok(Err(e)) => panic!("error: {e}"),
		Err(_) => panic!("timed out waiting for lane message"),
	}

	Ok(())
}

async fn test_lane_local_fast_path_delivery(pubsub: &PubSub) -> Result<()> {
	tracing::info!("testing lane local fast-path delivery");

	let suffix = Uuid::new_v4();
	let subject = format!("test.lane.local.{suffix}");
	let lane = pubsub.named_lane(
		format!("test-lane-local-{suffix}"),
		integration_lane_spec(true),
	)?;
	let mut subscriber = lane.subscribe(&subject).await?;

	let payload = b"lane local message";
	let outcome = lane
		.publish(&subject, payload.to_vec(), LanePublishOpts::default())
		.await;
	assert!(outcome.accepted);

	expect_lane_message(&mut subscriber, payload).await?;

	Ok(())
}

async fn test_lane_driver_broadcast_delivery(pubsub: &PubSub) -> Result<()> {
	tracing::info!("testing lane driver broadcast delivery");

	let suffix = Uuid::new_v4();
	let subject = format!("test.lane.driver.{suffix}");
	let lane = pubsub.named_lane(
		format!("test-lane-driver-{suffix}"),
		integration_lane_spec(false),
	)?;
	let mut sub1 = lane.subscribe(&subject).await?;
	let mut sub2 = lane.subscribe(&subject).await?;

	let payload = b"lane driver broadcast message";
	let outcome = lane
		.publish(&subject, payload.to_vec(), LanePublishOpts::default())
		.await;
	assert!(outcome.accepted);

	expect_lane_message(&mut sub1, payload).await?;
	expect_lane_message(&mut sub2, payload).await?;
