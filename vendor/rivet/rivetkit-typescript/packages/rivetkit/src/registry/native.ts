...
type NativeBroadcastPressureHandler = (pressure: BroadcastPressure) => void;
type NativeBroadcastPressureState = {
	pressure: BroadcastPressure;
	handlers: Set<NativeBroadcastPressureHandler>;
};
type NativeActorRuntimeState = {
	...
	broadcastPressure?: NativeBroadcastPressureState;
};
type NativeBroadcastPressureInput = {
	readonly credit: number;
	readonly queueDepth: number;
	readonly oldestAgeMs: number | bigint | null;
};
const MAX_BROADCAST_PRESSURE_U32 = 0xffffffff;
const DEFAULT_BROADCAST_PRESSURE: BroadcastPressure = Object.freeze({
	credit: MAX_BROADCAST_PRESSURE_U32,
	queueDepth: 0,
	oldestAgeMs: null,
});
...
function cloneBroadcastPressure(
	pressure: BroadcastPressure,
): BroadcastPressure {
	return {
		credit: pressure.credit,
		queueDepth: pressure.queueDepth,
		oldestAgeMs: pressure.oldestAgeMs,
	};
}

function normalizeBroadcastPressureNumber(
	value: number | bigint,
	max: number,
): number {
	if (typeof value === "bigint") {
		...
		return Number(value > maxBigInt ? maxBigInt : value);
	}
...
function normalizeBroadcastPressure(
	pressure: NativeBroadcastPressureInput,
): BroadcastPressure {
	return {
		credit: normalizeBroadcastPressureNumber(
			pressure.credit,
			MAX_BROADCAST_PRESSURE_U32,
		),
		queueDepth: normalizeBroadcastPressureNumber(
			pressure.queueDepth,
			MAX_BROADCAST_PRESSURE_U32,
		),
		oldestAgeMs:
			pressure.oldestAgeMs === null
				? null
				: normalizeBroadcastPressureNumber(
...
function getNativeBroadcastPressureState(
	runtime: CoreRuntime,
	ctx: ActorContextHandle,
): NativeBroadcastPressureState {
	const runtimeState = getNativeRuntimeState(runtime, ctx);
	if (!runtimeState.broadcastPressure) {
		runtimeState.broadcastPressure = {
			pressure: cloneBroadcastPressure(DEFAULT_BROADCAST_PRESSURE),
			handlers: new Set(),
		};
	}
	return runtimeState.broadcastPressure;
}

export function applyNativeBroadcastPressure(
	runtime: CoreRuntime,
	ctx: ActorContextHandle,
	pressure: NativeBroadcastPressureInput,
): void {
	const pressureState = getNativeBroadcastPressureState(runtime, ctx);
	pressureState.pressure = normalizeBroadcastPressure(pressure);

	for (const handler of [...pressureState.handlers]) {
		try {
			handler(cloneBroadcastPressure(pressureState.pressure));
		} catch (error) {
			logger().error({
				msg: "broadcast pressure handler failed",
				error,
			});
		}
	}
...
export class ActorContextHandleAdapter {
	...
	broadcastBudget(): BroadcastPressure {
		return cloneBroadcastPressure(
			getNativeBroadcastPressureState(this.#runtime, this.#ctx).pressure,
		);
	}

	onPressure(handler: (pressure: BroadcastPressure) => void): () => void {
		const pressureState = getNativeBroadcastPressureState(
			this.#runtime,
			this.#ctx,
		);
		pressureState.handlers.add(handler);
		return () => {
			pressureState.handlers.delete(handler);
		};
	}
...
export function buildNativeFactory(
	...
	const callbacks = {
		...
		onBroadcastPressure: wrapNativeCallback(
			async (
				error: unknown,
				payload: {
					ctx: ActorContextHandle;
				...
				const { ctx, pressure } = unwrapTsfnPayload(error, payload);
				applyNativeBroadcastPressure(runtime, ctx, pressure);
			},
		),
	};
