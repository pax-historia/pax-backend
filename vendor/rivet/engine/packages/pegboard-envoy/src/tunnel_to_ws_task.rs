...
const TUNNEL_PRESSURE_PROTOCOL_VERSION: u16 = 6;
...
async fn handle_message(
	...
	let to_client_msg = match msg {
		...
		protocol::ToEnvoyConn::ToEnvoyTunnelPressure(x) => {
			if conn.protocol_version < TUNNEL_PRESSURE_PROTOCOL_VERSION {
				tracing::debug!(
					protocol_version = conn.protocol_version,
					required_version = TUNNEL_PRESSURE_PROTOCOL_VERSION,
					"dropping tunnel pressure for older envoy protocol"
				);
				return Ok(false);
			}

			protocol::ToEnvoy::ToEnvoyTunnelPressure(x)
		}
	};
