...
pub async fn pegboard_runner_drain_older_versions(
	...
	// Scan both legacy default-lane and lane-aware indexes for older versions.
	let older_runners = ctx
		...
		.run(|tx| async move {
			...
			while let Some(entry) = stream.try_next().await? {
				...
				if key.version < input.version && !older_runners.contains(&data.workflow_id) {
					older_runners.push(data.workflow_id);
				}
			}

			let runner_lane_alloc_subspace =
				keys::subspace().subspace(&keys::ns::RunnerLaneAllocIdxKey::name_subspace(
					input.namespace_id,
					input.name.clone(),
				));
			...
			while let Some(entry) = stream.try_next().await? {
				let (key, data) = tx.read_entry::<keys::ns::RunnerLaneAllocIdxKey>(&entry)?;
