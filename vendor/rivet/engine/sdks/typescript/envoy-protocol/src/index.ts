...
function read22(bc: bare.ByteCursor): RequestId | null {
    return bare.readBool(bc) ? readRequestId(bc) : null
}

function write22(bc: bare.ByteCursor, x: RequestId | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writeRequestId(bc, x)
    }
...
export type ToEnvoyTunnelPressure = {
    readonly gatewayId: GatewayId
    readonly requestId: RequestId | null
    readonly pressure: Pressure
}

export function readToEnvoyTunnelPressure(bc: bare.ByteCursor): ToEnvoyTunnelPressure {
    return {
        gatewayId: readGatewayId(bc),
        requestId: read22(bc),
...
export function writeToEnvoyTunnelPressure(bc: bare.ByteCursor, x: ToEnvoyTunnelPressure): void {
    writeGatewayId(bc, x.gatewayId)
    write22(bc, x.requestId)
...
function read25(bc: bare.ByteCursor): Json | null {
    return bare.readBool(bc) ? readJson(bc) : null
}

function write25(bc: bare.ByteCursor, x: Json | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writeJson(bc, x)
    }
...
function read26(bc: bare.ByteCursor): readonly ActorCheckpoint[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write26(bc: bare.ByteCursor, x: readonly ActorCheckpoint[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
export type ToEnvoy =
    ...
    | { readonly tag: "ToEnvoyTunnelPressure"; readonly val: ToEnvoyTunnelPressure }
...
export function readToEnvoy(bc: bare.ByteCursor): ToEnvoy {
    ...
    switch (tag) {
        ...
        case 10:
            return { tag: "ToEnvoyTunnelPressure", val: readToEnvoyTunnelPressure(bc) }
        default: {
            bc.offset = offset
            throw new bare.BareError(offset, "invalid tag")
        }
...
export function writeToEnvoy(bc: bare.ByteCursor, x: ToEnvoy): void {
    switch (x.tag) {
        ...
        case "ToEnvoyTunnelPressure": {
            bare.writeU8(bc, 10)
            writeToEnvoyTunnelPressure(bc, x.val)
            break
        }
    }
}
