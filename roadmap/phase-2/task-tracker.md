...
| 4 | **No-fault medium run** — Run the hello-world workload on Fly at roughly 100 concurrent games for 30 minutes with the no-fault profile, then verify all 17 guarantee oracles are green and record the run artifacts. | `in_progress` — runner support and a dedicated 100-game no-fault workload are in place; next step is the Fly run and oracle proof. |
