...
async fn handle_conn_message(
	...
	match message {
		...
		protocol::ToEnvoy::ToEnvoyTunnelPressure(pressure) => {
			let actor_id = pressure.request_id.as_ref().and_then(|request_id| {
				ctx.request_to_actor
					.get(&[&pressure.gateway_id, request_id])
					.cloned()
			});
			ctx.shared.config.callbacks.on_tunnel_pressure(
				EnvoyHandle::from_shared(ctx.shared.clone()),
				actor_id,
				pressure,
			);
		}
...
mod tests {
	...
	struct PressureCallbacks {
		seen: Arc<StdMutex<Vec<(Option<String>, protocol::ToEnvoyTunnelPressure)>>>,
	}

	impl EnvoyCallbacks for PressureCallbacks {
		fn on_tunnel_pressure(
			&self,
			_handle: EnvoyHandle,
			actor_id: Option<String>,
			pressure: protocol::ToEnvoyTunnelPressure,
		) {
			self.seen
				.lock()
				.expect("pressure callbacks poisoned")
				.push((actor_id, pressure));
		}
		...
		fn fetch(
			...
			Box::pin(async { anyhow::bail!("fetch should not be called in pressure tests") })
		}
		...
		fn websocket(
			...
			Box::pin(async { anyhow::bail!("websocket should not be called in pressure tests") })
		}
	...
	fn new_envoy_context(callbacks: Arc<dyn EnvoyCallbacks>) -> EnvoyContext {
	...
	#[tokio::test]
	async fn tunnel_pressure_message_invokes_callback() {
		let seen = Arc::new(StdMutex::new(Vec::new()));
		let callbacks = Arc::new(PressureCallbacks { seen: seen.clone() });
		let mut ctx = new_envoy_context(callbacks);
		let (start_tx, _start_rx) = tokio::sync::watch::channel(());

		let gateway_id = [1, 2, 3, 4];
		let request_id = [5, 6, 7, 8];
		ctx.request_to_actor
			.insert(&[&gateway_id, &request_id], "actor-a".to_owned());
		let lost_timeout = handle_conn_message(
			&mut ctx,
			&start_tx,
			None,
			protocol::ToEnvoy::ToEnvoyTunnelPressure(protocol::ToEnvoyTunnelPressure {
				gateway_id,
		...
		assert!(lost_timeout.is_none());
		let seen = seen.lock().expect("pressure callbacks poisoned");
		assert_eq!(seen.len(), 1);
		assert_eq!(seen[0].0.as_deref(), Some("actor-a"));
		assert_eq!(seen[0].1.gateway_id, gateway_id);
		assert_eq!(seen[0].1.request_id, Some(request_id));
		assert_eq!(seen[0].1.pressure.credit, 0);
		assert_eq!(seen[0].1.pressure.queue_depth, 16);
		assert_eq!(seen[0].1.pressure.oldest_age_ms, Some(250));
	}
}
