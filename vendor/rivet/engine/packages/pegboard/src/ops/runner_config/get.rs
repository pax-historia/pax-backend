...
const RUNNER_CONFIG_CACHE_TTL_MS: i64 = 30_000;
...
pub async fn pegboard_runner_config_get(
	...
	if input.bypass_cache {
		...
		ctx.cache()
			.clone()
			.request()
			// Upserts and metadata refreshes explicitly purge this cache; keep it
			// warm through actor setup bursts instead of re-reading per wave.
			.ttl(RUNNER_CONFIG_CACHE_TTL_MS)
