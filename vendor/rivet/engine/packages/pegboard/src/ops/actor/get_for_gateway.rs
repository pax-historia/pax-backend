...
pub struct Output {
	...
	pub routing_generation: Option<u32>,
	pub version: u32,
}
...
pub async fn pegboard_actor_get_for_gateway(
	...
	ctx.udb()?
		.run(|tx| async move {
			...
			let version = version_entry.unwrap_or(1);

			let routing_generation = match (version, runner_id, envoy_key.as_ref()) {
				(1, Some(runner_id), _) => {
					tx.read_opt(
						&keys::runner::ActorKey::new(runner_id, input.actor_id),
						Serializable,
					)
					.await?
				}
