	...
	ctx.udb()?
		.run(|tx| async move {
			...
			let old_lane_alloc_key = keys::ns::RunnerLaneAllocIdxKey::new(
				input.namespace_id,
				input.name.clone(),
				old_worker_lane.as_str().to_owned(),
				input.version,
				remaining_millislots,
				last_ping_ts,
				input.runner_id,
			...
			tx.delete(&legacy_alloc_key);
			tx.delete(&old_lane_alloc_key);
		...
		.map(|(queue_key, generation)| async move {
			...
			ctx.udb()?
				.run(|tx| async move {
					...
					let WorkerLanePlacement::Selected(assignment) = place_actor_in_worker_lane(
