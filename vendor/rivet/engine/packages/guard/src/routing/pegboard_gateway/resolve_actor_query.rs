...
const SLOW_GET_OR_CREATE_HOP: Duration = Duration::from_millis(500);
const SLOW_GET_OR_CREATE_TOTAL: Duration = Duration::from_millis(1000);
	...
	if target_dc_label == ctx.config().dc_label() {
			...
			.await
		{
			Ok(res) => {
				...
				Ok(ResolveQueryActorResult::Found {
...
fn log_slow_get_or_create_hop(
	...
	if duration < SLOW_GET_OR_CREATE_HOP {
		return;
	}
	...
	tracing::warn!(
		...
		actor_name,
		pool_name,
		get_or_create_hop = hop,
		get_or_create_hop_duration_ms = duration.as_millis(),
		"slow guard get_or_create hop"
	);
}

fn log_slow_get_or_create_total(
	namespace_name: &str,
	actor_name: &str,
	pool_name: &str,
	result: &'static str,
	...
	if duration < SLOW_GET_OR_CREATE_TOTAL {
		return;
	}
	...
	tracing::warn!(
		...
		actor_name,
		pool_name,
		get_or_create_result = result,
		get_or_create_duration_ms = duration.as_millis(),
		"slow guard get_or_create"
	);
}
