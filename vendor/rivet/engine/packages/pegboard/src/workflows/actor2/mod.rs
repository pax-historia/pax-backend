	...
	if input.from_v1 {
		...
		ctx.msg(CreateComplete {
			actor: Some(Actor {
				...
				runner_name_selector: input.pool_name.clone(),
	...
	let create_ts = ctx
		...
		.run(|tx| async move {
			...
			if let Some(lane_hint) = &input.lane_hint {
				tx.write(
					&crate::keys::actor::LaneHintKey::new(input.actor_id),
					lane_hint.clone(),
				)?;
			}
			tx.write(
...
async fn update_state_and_db(ctx: &ActivityCtx, input: &UpdateStateAndDbInput) -> Result<()> {
	...
	let ups = ctx.ups()?;
	crate::routing_directory::publish_delta_best_effort(
		&ups,
		crate::routing_directory::RoutingDelta::Removed {
			actor_id,
			generation: u64::MAX,
		},
	)
