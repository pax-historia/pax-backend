...
| 3 | **Core state, blob, and migration adapter** — Implement the `GameContext` adapter over `c.*`, the compact `c.state` working-set layout, `c.blob` snapshot layout, and the `historia:v1` through `historia:v5` migration dispatch. | `in_progress` |
