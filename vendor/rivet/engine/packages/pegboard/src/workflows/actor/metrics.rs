...
pub(crate) async fn pegboard_actor_metrics(ctx: &mut WorkflowCtx, input: &Input) -> Result<()> {
	...
	ctx.loope(LifecycleState::new(start_paused), |ctx, state| {
		async move {
			...
			// The KV storage scan is periodic bookkeeping. Running it on every pause/resume
			// signal creates a setup-time activity storm when many actors become Ready at once.
			if sigs.is_empty() || destroy {
	...
	*state = Some(State {
	...
	ctx.udb()?
		.run(|tx| async move {
			...
			let is_sleeping = tx
				.exists(&keys::actor::SleepTsKey::new(input.actor_id), Serializable)
				.await?;
			let is_destroyed = tx
				.exists(
					&keys::actor::DestroyTsKey::new(input.actor_id),
					Serializable,
				)
				.await?;
			...
			if !is_destroyed && !tx.exists(&metric_incremented_key, Serializable).await? {
