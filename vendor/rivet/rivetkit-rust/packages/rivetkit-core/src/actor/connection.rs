...
pub(crate) struct OutgoingEvent {
	inner: Arc<OutgoingEventInner>,
}

struct OutgoingEventInner {
	name: String,
	args: Vec<u8>,
	cached_payloads: Mutex<BTreeMap<&'static str, Arc<[u8]>>>,
}

impl OutgoingEvent {
	pub(crate) fn new(name: impl Into<String>, args: impl Into<Vec<u8>>) -> Self {
		Self {
			inner: Arc::new(OutgoingEventInner {
				name: name.into(),
				args: args.into(),
				cached_payloads: Mutex::new(BTreeMap::new()),
			}),
		}
	}
	...
	pub(crate) fn cached_payload(
		...
		encode: impl FnOnce(&str, &[u8]) -> Result<Vec<u8>>,
	) -> Result<Arc<[u8]>> {
		let mut cached_payloads = self.inner.cached_payloads.lock();
		if let Some(payload) = cached_payloads.get(cache_key) {
			return Ok(payload.clone());
		}

		let payload: Arc<[u8]> = encode(self.name(), self.args())?.into();
		cached_payloads.insert(cache_key, payload.clone());
		Ok(payload)
	}
}
...
impl ConnHandle {
	...
	pub(crate) fn try_send(&self, name: &str, args: &[u8]) -> Result<()> {
		self.try_send_event(OutgoingEvent::new(name, args))
	}

	pub(crate) fn try_send_event(&self, event: OutgoingEvent) -> Result<()> {
