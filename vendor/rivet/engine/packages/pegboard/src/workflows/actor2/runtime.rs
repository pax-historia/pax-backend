...
use crate::{
	executor::{
		WorkerLane, WorkerLaneEndpointCandidate, WorkerLaneEndpointPlacement,
		WorkerLanePlacementInput, actor_placement_key, place_actor_on_worker_endpoint,
	},
	keys,
			...
			async move {
				...
				let (allocation, error) = if acquired_slot {
					if is_serverless {
						...
						let lb_subspace = keys::subspace().subspace(
							&keys::ns::EnvoyLoadBalancerIdxKey::subspace(
								namespace_id,
								pool_name.clone(),
							),
						);
						...
						let endpoint_candidates = candidates
							.iter()
							.map(|lb_key| WorkerLaneEndpointCandidate {
								endpoint_key: lb_key.envoy_key.clone(),
						...
						if let WorkerLaneEndpointPlacement::Selected(assignment) =
							place_actor_on_worker_endpoint(
								WorkerLanePlacementInput {
									actor_key: &placement_key,
									lane: &worker_lane,
									now_ts: util::timestamp::now(),
									runner_eligible_threshold: envoy_eligible_threshold,
								},
								&endpoint_candidates,
							) {
							(
								Some(Allocation::Serverful {
									envoy_key: assignment.endpoint_key,
								}),
								None,
							)
						} else {
							tracing::debug!(
								...
								"no worker lane endpoint placement candidate"
							);
