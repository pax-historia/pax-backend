...
| # | Task | Progress |
|---|---|---|
| 1 | **Surface audit** — Re-read this phase's directive/exit signal and the local build/smoke surfaces (`scripts/dev/*`, `testing/smoke-bot`, the smoke target, the build matrix). Expand this placeholder into a concrete task list. | `to_do` |
| 2 | **Phase verification** — Re-read this phase's directive and exit signal in the [README](../README.md). Confirm `pnpm smoke` is green on this Mac on the new runtime with no production `.env` and audits are clean or logged. If anything is missing, add rows above this one and rerun. | `to_do` |
