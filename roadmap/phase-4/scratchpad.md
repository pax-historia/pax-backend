...
## 2026-05-28 11:14 PDT

Started Task 5 by making the existing `sleep-wake` workload phase executable: it now closes the current session set, waits the configured idle interval, and reopens the same game/player pairs. This unblocks the existing `shard-death-resilience` workload and gives race scenarios a deterministic reconnect-churn primitive.

Added `race-and-deploy-adversarial` plus `race-edge-probe-v1`/`v2` bundles. The scenario flips a live game from v1 to v2, requests sleep, queues a host event during the sleep transition, wakes the sleeping game with another host event, and churns reconnects through the sleep-grace window. The first live run found a real host-event durability gap: the synthetic `__pax_host_event__` wake connection was refused by the allowed-player gate, so sleeping games did not wake to drain queued host events. After allowing that synthetic wake path, the next run found a second race: the host-event drain loop could mark events delivered before the child had loaded the bundle/onWake path, so the bundle never handled them. The parent now refreshes stale in-memory bundle state from the game record before a cold wake, allows synthetic host-event wake without creating a player session, and drains host events only after `onWake.sent`.

Verification: `pnpm typecheck`, `git diff --check`, bundle-specific typechecks/builds for both race probe bundles, parent/scenario-runner/oracles typechecks, and a live `race-and-deploy-adversarial --backend live --oracles scenario` run all passed. The passing live run used `--game-id-prefix race-deploy-v4`; `var/phase-4/race-and-deploy.result.json` shows all selected guarantees plus `G0_race_and_deploy_adversarial` passing. Partition nemesis coverage is still outstanding for Task 5.

## 2026-05-28 11:22 PDT

Finished the partition half of Task 5. The scenario-runner now has a one-shot `api-kind-partition` nemesis action: it reads the current API-kind registration, rewires the kind through `POST /admin/api-kinds` to an unroutable URL for a fixed window, records `nemesis.api-kind-partition.injected`, and restores the prior registration with `nemesis.api-kind-partition.restored` even during runner cleanup. The first concrete profile is `api-kind-partition-burst`, targeting `mock-ai.v1` for a short URL-service/provider partition.

Added `api-partition-adversarial` using the existing `hello-ai-call` bundle. It sends active player traffic across the partition window and the scenario-local oracle requires: the injection and restore records, at least one `api.invoke.response` with `ok=false`, `error=providerError`, and `statusCode=0` during the window, and at least one successful `mock-ai.v1` response after restoration. This keeps the partition guarantee at the gateway/URL-service boundary and avoids adding any billing-shaped or tenant-shaped primitive.

Verification: `pnpm --filter @pax-backend/scenario-runner check-types`, `pnpm --filter @pax-backend/oracles-lib check-types`, `pnpm --filter @pax-backend/bundle-hello-ai-call build`, root `pnpm typecheck`, `git diff --check`, and a live `api-partition-adversarial --backend live --oracles scenario` run all passed. The live run used `--game-id-prefix api-partition-v1`; `var/phase-4/api-partition.result.json` shows G5, G8, G14, and `G0_api_partition_adversarial` passing.

## 2026-05-28 11:30 PDT

Finished the CI release-gate wiring. Added `.github/workflows/scenario-suite.yml`, which runs on push, pull request, and manual dispatch, builds the local macOS stack prerequisites, and executes `scripts/test/scenario-suite-local.sh` for both child runtimes against `testing/scenarios` with every nemesis profile. The workflow uploads `var/scenario-suite`, local service logs, merged history, and API wire records on failure so failed cases keep their result JSON and history.

Two runner details changed to make the full matrix usable. First, `NemesisRuntime` now writes `nemesis.profile.started` and treats `await-nemesis` for an action not present in the selected profile as `nemesis.await.skipped` rather than a timeout; this lets specialized scenarios run under orthogonal nemeses. Second, the local suite script now passes explicit control/router/gateway URLs so suite result histories include the parent/shard history collected from the control plane. The partition scenario oracle is profile-aware: under non-partition nemeses it asserts baseline API success and no providerError, while under `api-kind-partition-burst` it asserts the typed partition failure and recovery path.
