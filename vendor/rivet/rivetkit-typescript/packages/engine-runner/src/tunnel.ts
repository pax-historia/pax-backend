...
import { decodeToClientTickWave } from "./tunnel-wave";
...
export class Tunnel {
	...
	async handleTickWave(wave: protocol.TickWave) {
		for (const message of decodeToClientTickWave(wave)) {
			await this.handleTunnelMessage(message);
		}
	}
