...
use crate::{
	http_client,
	keys::{self, KvBallotKey, KvValueKey, LegacyCommittedValueKey},
	metrics,
	replica::{
...
pub async fn epoxy_propose(ctx: &OperationCtx, input: &Input) -> Result<ProposalResult> {
	...
	if quorum_members.len() == 1 && quorum_members[0] == replica_id {
		if let Some(result) = try_commit_single_replica_fresh(ctx, replica_id, &proposal).await? {
			metrics::FAST_PATH_TOTAL.inc();
			metrics::PROPOSAL_DURATION.observe(start.elapsed().as_secs_f64());
			metrics::record_proposal_result(match &result {
...
async fn try_commit_single_replica_fresh(
	ctx: &OperationCtx,
	replica_id: ReplicaId,
	proposal: &SetProposal,
) -> Result<Option<ProposalResult>> {
	let proposal_template = proposal.clone();

	ctx.udb()?
		.run(move |tx| {
			let proposal = proposal_template.clone();
			async move {
				let subspace = keys::subspace(replica_id);
				let legacy_subspace = keys::legacy_subspace(replica_id);
				let value_key = KvValueKey::new(proposal.key.clone());
				let legacy_value_key = LegacyCommittedValueKey::new(proposal.key.clone());
				let legacy_v2_value_key = KvValueKey::new(proposal.key.clone());
				let ballot_key = KvBallotKey::new(proposal.key.clone());
				...
				let (
					...
					tx.get(&packed_ballot_key, Serializable),
				)?;
				...
				{
					if value.mutable && proposal.mutable {
						return Ok(None);
					}

					return Ok(Some(proposal.result_for_committed_value(value.value)));
				}

				// If a previous proposal left a ballot behind, the normal prepare path is still
				// required to preserve any accepted value before choosing a new one.
				if current_ballot.is_some() {
					return Ok(None);
				}

				let ballot = Ballot::new(1, replica_id);
				ballot::store_ballot(&tx, replica_id, proposal.key.clone(), ballot)?;

				match commit_kv::commit_kv(
					&tx,
					replica_id,
					proposal.key.clone(),
				...
				.await?
				{
					CommitKvOutcome::Committed => Ok(Some(ProposalResult::Committed)),
					CommitKvOutcome::AlreadyCommitted { value, .. } => {
						Ok(Some(proposal.result_for_committed_value(value)))
					}
					CommitKvOutcome::StaleBallot { .. } => Ok(None),
				}
			}
		})
		.custom_instrument(tracing::info_span!("single_replica_commit_tx"))
		.await
}
