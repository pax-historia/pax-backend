...
const SLOW_WORKFLOW_ACTIVITY_PHASE_MS: u128 = 1000;

fn log_slow_workflow_activity_phase(
	workflow_id: Id,
	workflow_name: &str,
	activity_name: &'static str,
	activity_phase: &'static str,
	duration_ms: u128,
) {
	if duration_ms >= SLOW_WORKFLOW_ACTIVITY_PHASE_MS {
		tracing::warn!(
			?workflow_id,
			workflow_name,
			activity_name,
			activity_phase,
			activity_phase_duration_ms = duration_ms,
			"slow workflow activity phase"
		);
	}
}
...
impl WorkflowCtx {
	...
	async fn run_activity<A: Activity>(
		...
		let activity_start_lag_ms = (rivet_util::timestamp::now() - create_ts).max(0) as u128;
		log_slow_workflow_activity_phase(
			self.workflow_id,
			&self.name,
			A::NAME,
			"start_lag",
			activity_start_lag_ms,
		);
		...
		let activity_body_duration = start_instant.elapsed();
		log_slow_workflow_activity_phase(
			self.workflow_id,
			&self.name,
			A::NAME,
			"body",
			activity_body_duration.as_millis(),
		);

		let dt = activity_body_duration.as_secs_f64();
...
impl WorkflowCtx {
	...
	/// Returns true when this workflow has no recorded events yet.
	pub fn is_first_run(&self) -> bool {
		self.event_history.is_empty()
	}

	/// Returns true when the cursor is currently positioned over recorded history.
	///
	/// This is useful for removed-step compatibility markers: old histories still need to consume
	/// a recorded event, while fresh histories can skip writing a marker for a step that no longer
	/// exists.
	pub fn has_current_history_event(&self) -> bool {
		self.cursor.current_event().is_some()
	}
