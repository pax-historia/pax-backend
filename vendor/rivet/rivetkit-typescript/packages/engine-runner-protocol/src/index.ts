...
export function readSequenceRange(bc: bare.ByteCursor): SequenceRange {
    return {
        first: bare.readU64(bc),
        last: bare.readU64(bc),
    }
}

export function writeSequenceRange(bc: bare.ByteCursor, x: SequenceRange): void {
    bare.writeU64(bc, x.first)
    bare.writeU64(bc, x.last)
}

export type Pressure = {
    readonly credit: u32
    readonly queueDepth: u32
    readonly oldestAgeMs: u64 | null
}

export function readPressure(bc: bare.ByteCursor): Pressure {
    return {
        credit: bare.readU32(bc),
        queueDepth: bare.readU32(bc),
        oldestAgeMs: read2(bc),
    }
}

export function writePressure(bc: bare.ByteCursor, x: Pressure): void {
    bare.writeU32(bc, x.credit)
    bare.writeU32(bc, x.queueDepth)
    write2(bc, x.oldestAgeMs)
}
...
export function readTunnelFrameKind(bc: bare.ByteCursor): TunnelFrameKind {
    const offset = bc.offset
    const tag = bare.readU8(bc)
    switch (tag) {
        case 0:
            return TunnelFrameKind.Http
        case 1:
            return TunnelFrameKind.WebSocket
        case 2:
            return TunnelFrameKind.Action
        case 3:
            return TunnelFrameKind.Lifecycle
        default: {
            bc.offset = offset
            throw new bare.BareError(offset, "invalid tag")
        }
...
export function writeTunnelFrameKind(bc: bare.ByteCursor, x: TunnelFrameKind): void {
    switch (x) {
        case TunnelFrameKind.Http: {
            bare.writeU8(bc, 0)
            break
        }
        case TunnelFrameKind.WebSocket: {
            bare.writeU8(bc, 1)
            break
        }
        case TunnelFrameKind.Action: {
            bare.writeU8(bc, 2)
            break
        }
        case TunnelFrameKind.Lifecycle: {
            bare.writeU8(bc, 3)
            break
        }
    }
...
export type TunnelFrame = {
    ...
    readonly messageKind: TunnelFrameKind
    readonly bytes: ArrayBuffer
}

export function readTunnelFrame(bc: bare.ByteCursor): TunnelFrame {
    return {
        requestId: readRequestId(bc),
        sequenceRange: readSequenceRange(bc),
        messageKind: readTunnelFrameKind(bc),
        bytes: bare.readData(bc),
    }
}

export function writeTunnelFrame(bc: bare.ByteCursor, x: TunnelFrame): void {
    writeRequestId(bc, x.requestId)
    writeSequenceRange(bc, x.sequenceRange)
    writeTunnelFrameKind(bc, x.messageKind)
    bare.writeData(bc, x.bytes)
}

function read11(bc: bare.ByteCursor): readonly TunnelFrame[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
    const result = [readTunnelFrame(bc)]
    for (let i = 1; i < len; i++) {
        result[i] = readTunnelFrame(bc)
    }
    return result
}

function write11(bc: bare.ByteCursor, x: readonly TunnelFrame[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
        writeTunnelFrame(bc, x[i])
    }
}

function read12(bc: bare.ByteCursor): Pressure | null {
    return bare.readBool(bc) ? readPressure(bc) : null
}

function write12(bc: bare.ByteCursor, x: Pressure | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writePressure(bc, x)
    }
...
export type TickWave = {
    readonly epoch: u64
    readonly gatewayId: GatewayId
    readonly frames: readonly TunnelFrame[]
    readonly backpressure: Pressure | null
}

export function readTickWave(bc: bare.ByteCursor): TickWave {
    return {
        epoch: bare.readU64(bc),
        gatewayId: readGatewayId(bc),
        frames: read11(bc),
        backpressure: read12(bc),
    }
}

export function writeTickWave(bc: bare.ByteCursor, x: TickWave): void {
    bare.writeU64(bc, x.epoch)
    writeGatewayId(bc, x.gatewayId)
    write11(bc, x.frames)
    write12(bc, x.backpressure)
}

export type TunnelResume = {
    readonly gatewayId: GatewayId
    readonly requestId: RequestId
    readonly lastAckedSeq: u64
}

export function readTunnelResume(bc: bare.ByteCursor): TunnelResume {
    return {
        gatewayId: readGatewayId(bc),
        requestId: readRequestId(bc),
        lastAckedSeq: bare.readU64(bc),
    }
}

export function writeTunnelResume(bc: bare.ByteCursor, x: TunnelResume): void {
    writeGatewayId(bc, x.gatewayId)
    writeRequestId(bc, x.requestId)
    bare.writeU64(bc, x.lastAckedSeq)
}
...
export function readTunnelAck(bc: bare.ByteCursor): TunnelAck {
    return {
        gatewayId: readGatewayId(bc),
        requestId: readRequestId(bc),
        lastAckedSeq: bare.readU64(bc),
...
export function writeTunnelAck(bc: bare.ByteCursor, x: TunnelAck): void {
    writeGatewayId(bc, x.gatewayId)
    writeRequestId(bc, x.requestId)
    bare.writeU64(bc, x.lastAckedSeq)
}

function read13(bc: bare.ByteCursor): RequestId | null {
    return bare.readBool(bc) ? readRequestId(bc) : null
}

function write13(bc: bare.ByteCursor, x: RequestId | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writeRequestId(bc, x)
    }
...
export type TunnelPressure = {
    readonly gatewayId: GatewayId
    readonly requestId: RequestId | null
    readonly pressure: Pressure
}

export function readTunnelPressure(bc: bare.ByteCursor): TunnelPressure {
    return {
        gatewayId: readGatewayId(bc),
        requestId: read13(bc),
        pressure: readPressure(bc),
    }
}

export function writeTunnelPressure(bc: bare.ByteCursor, x: TunnelPressure): void {
    writeGatewayId(bc, x.gatewayId)
    write13(bc, x.requestId)
    writePressure(bc, x.pressure)
}

export type TunnelControl =
    | { readonly tag: "TunnelResume"; readonly val: TunnelResume }
    | { readonly tag: "TunnelAck"; readonly val: TunnelAck }
    | { readonly tag: "TunnelPressure"; readonly val: TunnelPressure }

export function readTunnelControl(bc: bare.ByteCursor): TunnelControl {
    const offset = bc.offset
    const tag = bare.readU8(bc)
    switch (tag) {
        case 0:
            return { tag: "TunnelResume", val: readTunnelResume(bc) }
        case 1:
            return { tag: "TunnelAck", val: readTunnelAck(bc) }
        case 2:
            return { tag: "TunnelPressure", val: readTunnelPressure(bc) }
        default: {
            bc.offset = offset
            throw new bare.BareError(offset, "invalid tag")
        }
...
export function writeTunnelControl(bc: bare.ByteCursor, x: TunnelControl): void {
    switch (x.tag) {
        case "TunnelResume": {
            bare.writeU8(bc, 0)
            writeTunnelResume(bc, x.val)
            break
        }
        case "TunnelAck": {
            bare.writeU8(bc, 1)
            writeTunnelAck(bc, x.val)
            break
        }
        case "TunnelPressure": {
            bare.writeU8(bc, 2)
            writeTunnelPressure(bc, x.val)
            break
        }
    }
}

export type ToServerTickWave = {
    readonly wave: TickWave
}

export function readToServerTickWave(bc: bare.ByteCursor): ToServerTickWave {
    return {
        wave: readTickWave(bc),
    }
}

export function writeToServerTickWave(bc: bare.ByteCursor, x: ToServerTickWave): void {
    writeTickWave(bc, x.wave)
}

export type ToServerTunnelControl = {
    readonly control: TunnelControl
}

export function readToServerTunnelControl(bc: bare.ByteCursor): ToServerTunnelControl {
    return {
        control: readTunnelControl(bc),
    }
}

export function writeToServerTunnelControl(bc: bare.ByteCursor, x: ToServerTunnelControl): void {
    writeTunnelControl(bc, x.control)
}

export type ToClientTickWave = {
    readonly wave: TickWave
}

export function readToClientTickWave(bc: bare.ByteCursor): ToClientTickWave {
    return {
        wave: readTickWave(bc),
    }
}

export function writeToClientTickWave(bc: bare.ByteCursor, x: ToClientTickWave): void {
    writeTickWave(bc, x.wave)
}

export type ToClientTunnelControl = {
    readonly control: TunnelControl
}

export function readToClientTunnelControl(bc: bare.ByteCursor): ToClientTunnelControl {
    return {
        control: readTunnelControl(bc),
    }
}

export function writeToClientTunnelControl(bc: bare.ByteCursor, x: ToClientTunnelControl): void {
    writeTunnelControl(bc, x.control)
}
...
function read14(bc: bare.ByteCursor): ReadonlyMap<string, ActorName> {
    const len = bare.readUintSafe(bc)
...
function write14(bc: bare.ByteCursor, x: ReadonlyMap<string, ActorName>): void {
    bare.writeUintSafe(bc, x.size)
    for (const kv of x) {
        bare.writeString(bc, kv[0])
...
function read15(bc: bare.ByteCursor): ReadonlyMap<string, ActorName> | null {
    return bare.readBool(bc) ? read14(bc) : null
}

function write15(bc: bare.ByteCursor, x: ReadonlyMap<string, ActorName> | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        write14(bc, x)
    }
...
function read16(bc: bare.ByteCursor): Json | null {
    return bare.readBool(bc) ? readJson(bc) : null
}

function write16(bc: bare.ByteCursor, x: Json | null): void {
    bare.writeBool(bc, x != null)
    if (x != null) {
        writeJson(bc, x)
    }
...
export function readToServerInit(bc: bare.ByteCursor): ToServerInit {
    return {
        ...
        prepopulateActorNames: read15(bc),
        metadata: read16(bc),
    }
}
...
export function writeToServerInit(bc: bare.ByteCursor, x: ToServerInit): void {
    ...
    write15(bc, x.prepopulateActorNames)
...
function read17(bc: bare.ByteCursor): readonly ActorCheckpoint[] {
    const len = bare.readUintSafe(bc)
    if (len === 0) {
        return []
    }
...
function write17(bc: bare.ByteCursor, x: readonly ActorCheckpoint[]): void {
    bare.writeUintSafe(bc, x.length)
    for (let i = 0; i < x.length; i++) {
...
export type ToServer =
    ...
    | { readonly tag: "ToServerTickWave"; readonly val: ToServerTickWave }
    | { readonly tag: "ToServerTunnelControl"; readonly val: ToServerTunnelControl }
...
export function readToServer(bc: bare.ByteCursor): ToServer {
    ...
    switch (tag) {
        ...
        case 7:
            return { tag: "ToServerTickWave", val: readToServerTickWave(bc) }
        case 8:
            return { tag: "ToServerTunnelControl", val: readToServerTunnelControl(bc) }
        default: {
            bc.offset = offset
            throw new bare.BareError(offset, "invalid tag")
        }
...
export function writeToServer(bc: bare.ByteCursor, x: ToServer): void {
    switch (x.tag) {
        ...
        case "ToServerTickWave": {
            bare.writeU8(bc, 7)
            writeToServerTickWave(bc, x.val)
            break
        }
        case "ToServerTunnelControl": {
            bare.writeU8(bc, 8)
            writeToServerTunnelControl(bc, x.val)
            break
        }
    }
}
...
export type ToClient =
    ...
    | { readonly tag: "ToClientTickWave"; readonly val: ToClientTickWave }
    | { readonly tag: "ToClientTunnelControl"; readonly val: ToClientTunnelControl }
...
export function readToClient(bc: bare.ByteCursor): ToClient {
    ...
    switch (tag) {
        ...
        case 5:
            return { tag: "ToClientTickWave", val: readToClientTickWave(bc) }
        case 6:
            return { tag: "ToClientTunnelControl", val: readToClientTunnelControl(bc) }
        case 7:
...
export function writeToClient(bc: bare.ByteCursor, x: ToClient): void {
    switch (x.tag) {
        ...
        case "ToClientTickWave": {
            bare.writeU8(bc, 5)
            writeToClientTickWave(bc, x.val)
            break
        }
        case "ToClientTunnelControl": {
            bare.writeU8(bc, 6)
            writeToClientTunnelControl(bc, x.val)
            break
        }
        case "ToClientPing": {
            bare.writeU8(bc, 7)
...
export function encodeToServerTunnelMessageKind(x: ToServerTunnelMessageKind, config?: Partial<bare.Config>): Uint8Array {
    const fullConfig = config != null ? bare.Config(config) : DEFAULT_CONFIG
    const bc = new bare.ByteCursor(
        new Uint8Array(fullConfig.initialBufferLength),
        fullConfig,
    )
    writeToServerTunnelMessageKind(bc, x)
    return new Uint8Array(bc.view.buffer, bc.view.byteOffset, bc.offset)
}

export function decodeToServerTunnelMessageKind(bytes: Uint8Array): ToServerTunnelMessageKind {
    const bc = new bare.ByteCursor(bytes, DEFAULT_CONFIG)
    const result = readToServerTunnelMessageKind(bc)
    if (bc.offset < bc.view.byteLength) {
        throw new bare.BareError(bc.offset, "remaining bytes")
    }
    return result
...
export function encodeToClientTunnelMessageKind(x: ToClientTunnelMessageKind, config?: Partial<bare.Config>): Uint8Array {
    const fullConfig = config != null ? bare.Config(config) : DEFAULT_CONFIG
    const bc = new bare.ByteCursor(
        new Uint8Array(fullConfig.initialBufferLength),
        fullConfig,
    )
    writeToClientTunnelMessageKind(bc, x)
    return new Uint8Array(bc.view.buffer, bc.view.byteOffset, bc.offset)
}

export function decodeToClientTunnelMessageKind(bytes: Uint8Array): ToClientTunnelMessageKind {
    const bc = new bare.ByteCursor(bytes, DEFAULT_CONFIG)
    const result = readToClientTunnelMessageKind(bc)
    if (bc.offset < bc.view.byteLength) {
        throw new bare.BareError(bc.offset, "remaining bytes")
    }
    return result
