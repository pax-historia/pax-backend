...
export function stringifyToServer(message: protocol.ToServer): string {
	switch (message.tag) {
		...
		case "ToServerTickWave": {
			const { wave } = message.val;
			return `ToServerTickWave{epoch: ${stringifyBigInt(wave.epoch)}, gatewayId: ${idToStr(wave.gatewayId)}, frames: ${wave.frames.length}}`;
		}
		case "ToServerTunnelControl": {
			return `ToServerTunnelControl{control: ${message.val.control.tag}}`;
		}
	}
}
...
export function stringifyToClient(message: protocol.ToClient): string {
	switch (message.tag) {
		...
		case "ToClientTickWave": {
			const { wave } = message.val;
			return `ToClientTickWave{epoch: ${stringifyBigInt(wave.epoch)}, gatewayId: ${idToStr(wave.gatewayId)}, frames: ${wave.frames.length}}`;
		}
		case "ToClientTunnelControl": {
			return `ToClientTunnelControl{control: ${message.val.control.tag}}`;
		}
	}
}
