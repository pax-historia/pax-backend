...
pub async fn build_runners_drain_lane_request(
	port: u16,
	path: runners::drain_lane::DrainLanePath,
	query: runners::drain_lane::DrainLaneQuery,
	request: runners::drain_lane::DrainLaneRequest,
) -> Result<reqwest::RequestBuilder> {
	let client = rivet_pools::reqwest::client().await?;
	Ok(client
		.post(format!(
...
pub async fn runners_drain_lane(
	port: u16,
	path: runners::drain_lane::DrainLanePath,
	query: runners::drain_lane::DrainLaneQuery,
	request: runners::drain_lane::DrainLaneRequest,
) -> Result<runners::drain_lane::DrainLaneResponse> {
	let request = build_runners_drain_lane_request(port, path, query, request).await?;
	let response = request.send().await?;
	parse_response(response).await
}
