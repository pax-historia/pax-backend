...
impl PegboardGateway2 {
	async fn handle_request_inner(
		...
		if let Err(err) = in_flight_req.send_message(message).await {
		...
		let response_start = match tokio::time::timeout(response_start_timeout, fut).await {
			Ok(Ok(response_start)) => response_start,
			Ok(Err(err)) => {
				in_flight_req.stop().await;
				return Err(err);
			}
			Err(_) => {
				...
				in_flight_req.stop().await;
				return Err(GatewayResponseStartTimeout.build());
			}
		};
	...
	async fn handle_websocket_inner(
		...
		let can_hibernate = if after_hibernation {
			...
			if let Err(err) = in_flight_req.send_message(open_message).await {
				in_flight_req.stop().await;
				return Err(err);
			}
			...
			let open_msg = match tokio::time::timeout(websocket_open_timeout, fut).await {
				Ok(Ok(open_msg)) => open_msg,
				Ok(Err(err)) => {
					in_flight_req.stop().await;
					return Err(err);
				}
