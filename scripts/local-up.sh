#!/usr/bin/env bash
# scripts/local-up.sh — start the full local substrate stack on this Mac.
#
# Brings up:
#   1. Local Redis (Docker container, single-port mapping)
#   2. rivet-engine (cached binary from scripts/build-engine.sh)
#   3. parent-actor Node process
#   4. placement-router Rust binary
#
# Run scripts/local-down.sh to stop everything (kills processes, removes
# Redis container). Logs land under ./var/local-up/<service>.log.
...
if [[ ! -x "$ENGINE_BIN" ]]; then
  err "rivet-engine not built. Run: ./scripts/build-engine.sh"
fi
...
if [[ ! -x "$ROUTER_BIN" ]]; then
  err "placement-router not built. Run: ./scripts/build-router.sh"
fi
...
if [[ ! -f "$VWS_DIST" || ! -f "$ER_DIST" || ! -f "$ERP_DIST" ]]; then
  warn "vendor rivetkit TS dist missing; running scripts/build-vendor-ts.sh"
  "$REPO_ROOT/scripts/build-vendor-ts.sh"
fi
ok "vendor rivetkit TS dist: ok"

# Creator bundles built? (hello-ws-echo → dist/bundle.js)
HELLO_BUNDLE_DIST="$REPO_ROOT/tooling/bundles/hello-ws-echo/dist/bundle.js"
if [[ ! -f "$HELLO_BUNDLE_DIST" || "${PAX_FORCE_BUNDLE_REBUILD:-0}" == "1" ]]; then
  warn "hello-ws-echo bundle not built; running scripts/build-bundles.sh"
  "$REPO_ROOT/scripts/build-bundles.sh"
fi
...
if [[ -f "$ENGINE_PIDFILE" ]] && kill -0 "$(cat "$ENGINE_PIDFILE")" 2>/dev/null; then
  ...
  ( cd "$REPO_ROOT" && \
    ...
    nohup ./node_modules/.bin/tsx scripts/spawn-engine.mts \
...
ok "logs:          $LOG_DIR/"
echo
echo "    Run smoke:  pnpm smoke   (or: node tooling/smoke-bot/src/smoke.mjs)"
echo "    Stop stack: ./scripts/local-down.sh"
