...
use rivetkit_core::{
	...
	ActorWorkKind, BindParam, BroadcastPressure, ColumnValue, CoreRegistry as NativeCoreRegistry,
...
struct WasmCallbacks {
	...
	on_broadcast_pressure: Option<Function>,
	actions: JsValue,
}
...
impl WasmCallbacks {
	fn new(callbacks: JsValue) -> Self {
		Self {
			...
			on_broadcast_pressure: function_property(&callbacks, "onBroadcastPressure"),
...
async fn dispatch_event(callbacks: &WasmCallbacks, ctx: &WasmActorContext, event: ActorEvent) {
	match event {
		...
		ActorEvent::BroadcastPressure { pressure } => {
			let Some(callback) = callbacks.on_broadcast_pressure.as_ref() else {
				return;
			};
			let result = async {
				let payload = object();
				...
				set_anyhow(
					&payload,
					"pressure",
					JsValue::from(build_pressure_object(pressure)?),
				)?;
				call_callback(callback, &payload.into()).await?;
				Ok::<_, anyhow::Error>(())
			}
			...
			if let Err(error) = result {
				console_error(&format!(
					"wasm broadcast pressure callback failed: {error:#}"
				));
			}
		}
...
fn build_pressure_object(pressure: BroadcastPressure) -> Result<Object> {
	let object = object();
	set_anyhow(&object, "credit", JsValue::from_f64(pressure.credit as f64))?;
	set_anyhow(
		&object,
		"queueDepth",
		JsValue::from_f64(pressure.queue_depth as f64),
	)?;
	set_anyhow(
		&object,
		"oldestAgeMs",
		pressure
			.oldest_age_ms
			.map(|age_ms| JsValue::from_f64(age_ms as f64))
			.unwrap_or(JsValue::NULL),
	)?;
	Ok(object)
}
