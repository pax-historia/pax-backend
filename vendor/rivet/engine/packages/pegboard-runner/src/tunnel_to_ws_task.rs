...
#[tracing::instrument(name="tunnel_to_ws_task", skip_all, fields(ray_id=?ctx.ray_id(), req_id=?ctx.req_id(), runner_id=?conn.runner_id, workflow_id=?conn.workflow_id, protocol_version=%conn.protocol_version))]
pub async fn task(
	ctx: StandaloneCtx,
	conn: Arc<Conn>,
	...
	let to_client_msg = match msg {
		...
		protocol::mk2::ToRunner::ToClientCommands(mut command_wrappers) => {
			for command_wrapper in &mut command_wrappers {
				...
				{
					// Generation 0 is the initial start; no hibernating requests can have
					// been restored for an actor that has never run.
					if command_wrapper.checkpoint.generation > 0 {
						let ids = ctx
		...
		protocol::mk2::ToRunner::ToClientTickWave(x) => {
			if conn.protocol_version < PROTOCOL_MK2_VERSION {
				let messages = runner_gateway::wave_to_client_messages_fair(&x.wave)
					.context("failed to decode Tunnel v2 client wave for runner")?;
				for message in messages {
					let _ = conn
						.authorized_tunnel_routes
						.insert_async(
							(message.message_id.gateway_id, message.message_id.request_id),
							(),
						)
						.await;
					send_to_client_mk2(
						conn,
						protocol::mk2::ToClient::ToClientTunnelMessage(message),
					)
					.await?;
				}
			...
			for frame in &x.wave.frames {
				let _ = conn
					.authorized_tunnel_routes
					.insert_async((x.wave.gateway_id, frame.request_id), ())
					.await;
			}

			protocol::mk2::ToClient::ToClientTickWave(x)
		}
		protocol::mk2::ToRunner::ToClientTunnelControl(x) => {
			if conn.protocol_version < PROTOCOL_MK2_VERSION {
				tracing::debug!(?x, "dropping v8 Tunnel control for pre-v8 client");
				return Ok(());
			}

			protocol::mk2::ToClient::ToClientTunnelControl(x)
		}
	};

	send_to_client_mk2(conn, to_client_msg).await
}

async fn send_to_client_mk2(conn: &Conn, to_client_msg: protocol::mk2::ToClient) -> Result<()> {
	let serialized_msg =
