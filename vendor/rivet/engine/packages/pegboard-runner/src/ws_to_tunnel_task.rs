...
use tunnel_fabric::runner_gateway::{self, RunnerToGatewayAdapter};
...
async fn handle_message_mk2(
	...
	match msg {
		...
		protocol::mk2::ToServer::ToServerTickWave(tick_wave) => {
			handle_tick_wave_mk2(
				&ctx.ups()
					.context("failed to get UPS instance for tunnel tick wave")?,
				ctx.config()
					.pegboard()
					.runner_max_response_payload_body_size(),
				&conn.authorized_tunnel_routes,
			...
			.await
			.context("failed to handle tunnel tick wave")?;
		}
		protocol::mk2::ToServer::ToServerTunnelControl(control) => {
			handle_tunnel_control_mk2(
				&ctx.ups()
					.context("failed to get UPS instance for tunnel control")?,
				&conn.authorized_tunnel_routes,
				control,
			)
			.await
			.context("failed to handle tunnel control")?;
		}
...
async fn handle_tunnel_message_mk2(
	...
	tunnel_v2_adapter: &Mutex<RunnerToGatewayAdapter>,
	authorized_tunnel_routes: &HashMap<(protocol::mk2::GatewayId, protocol::mk2::RequestId), ()>,
	msg: protocol::mk2::ToServerTunnelMessage,
) -> Result<()> {
	let waves = match {
		let mut adapter = tunnel_v2_adapter.lock().await;
		adapter.build_waves([msg], None)
	} {
		Ok(waves) => waves,
		Err(runner_gateway::RunnerGatewayError::PayloadTooLarge { .. }) => {
			return Err(errors::WsError::InvalidPacket("payload too large".to_string()).build());
		}
		Err(err) => return Err(err.into()),
	};
	for wave in waves {
		handle_tick_wave_mk2(ups, max_payload_size, authorized_tunnel_routes, wave).await?;
	}
...
#[tracing::instrument(skip_all)]
async fn handle_tick_wave_mk2(
	ups: &PubSub,
	max_payload_size: usize,
	authorized_tunnel_routes: &HashMap<(protocol::mk2::GatewayId, protocol::mk2::RequestId), ()>,
	wave: protocol::mk2::TickWave,
) -> Result<()> {
	let messages = runner_gateway::wave_to_tunnel_messages(&wave)
		.context("failed to decode Tunnel v2 wave for gateway")?;
	let mut clear_routes = Vec::new();
	let mut total_inner_data_len = 0usize;

	for msg in &messages {
		let route = (msg.message_id.gateway_id, msg.message_id.request_id);
		let inner_data_len = runner_gateway::tunnel_message_inner_data_len(&msg.message_kind);
		total_inner_data_len += inner_data_len;
		...
		if runner_gateway::should_clear_route(&msg.message_kind) {
			clear_routes.push(route);
		}
	}

	let gateway_reply_to = GatewayReceiverSubject::new(wave.gateway_id).to_string();
	let msg_serialized = versioned::ToGateway::wrap_latest(
		protocol::mk2::ToGateway::ToServerTickWave(protocol::mk2::ToServerTickWave { wave }),
	)
	.serialize_with_embedded_version(PROTOCOL_MK2_VERSION)
	.context("failed to serialize Tunnel v2 wave for gateway")?;

	tracing::trace!(
		inner_data_len = total_inner_data_len,
		serialized_len = msg_serialized.len(),
		message_count = messages.len(),
		"publishing Tunnel v2 wave to gateway"
	);
	...
	ups.publish(&gateway_reply_to, &msg_serialized, PublishOpts::one())
		...
		.with_context(|| {
			format!(
				"failed to publish Tunnel v2 wave to gateway reply topic: {}",
				gateway_reply_to
			)
		})?;
...
#[tracing::instrument(skip_all)]
async fn handle_tunnel_control_mk2(
	ups: &PubSub,
	authorized_tunnel_routes: &HashMap<(protocol::mk2::GatewayId, protocol::mk2::RequestId), ()>,
	control: protocol::mk2::ToServerTunnelControl,
) -> Result<()> {
	let route = tunnel_control_route(&control.control)?;

	if !authorized_tunnel_routes.contains_async(&route).await {
		return Err(
			errors::WsError::InvalidPacket("unauthorized tunnel control".to_string()).build(),
		);
	}

	let gateway_reply_to = GatewayReceiverSubject::new(route.0).to_string();
	let msg_serialized =
		versioned::ToGateway::wrap_latest(protocol::mk2::ToGateway::ToServerTunnelControl(control))
			.serialize_with_embedded_version(PROTOCOL_MK2_VERSION)
			.context("failed to serialize Tunnel v2 control for gateway")?;

	tracing::trace!(
		serialized_len = msg_serialized.len(),
		"publishing Tunnel v2 control to gateway"
	);
	...
	ups.publish(&gateway_reply_to, &msg_serialized, PublishOpts::one())
		...
		.with_context(|| {
			format!(
				"failed to publish Tunnel v2 control to gateway reply topic: {}",
				gateway_reply_to
			)
		})?;
...
fn tunnel_control_route(
	control: &protocol::mk2::TunnelControl,
) -> Result<(protocol::mk2::GatewayId, protocol::mk2::RequestId)> {
	match control {
		protocol::mk2::TunnelControl::TunnelResume(resume) => {
			Ok((resume.gateway_id, resume.request_id))
		}
		protocol::mk2::TunnelControl::TunnelAck(ack) => Ok((ack.gateway_id, ack.request_id)),
		protocol::mk2::TunnelControl::TunnelPressure(pressure) => {
			let Some(request_id) = pressure.request_id else {
				return Err(errors::WsError::InvalidPacket(
					"runner pressure control must include request_id".to_string(),
				)
				.build());
			};
			Ok((pressure.gateway_id, request_id))
		}
	}
}
