...
use crate::actor::messages::{ActorEvent, BroadcastPressure, Request, StateDelta};
...
pub(crate) struct ActorContextInner {
	...
	// Forced-sync: pressure snapshots are updated from envoy callbacks and read
	// synchronously from actor contexts.
	broadcast_pressure: RwLock<BroadcastPressure>,
...
impl ActorContext {
	...
	pub(crate) fn build(
		...
		let ctx = Self(Arc::new(ActorContextInner {
			...
			broadcast_pressure: RwLock::new(BroadcastPressure::default()),
	...
	pub fn broadcast_pressure(&self) -> BroadcastPressure {
		*self.0.broadcast_pressure.read()
	}

	pub(crate) fn apply_broadcast_pressure(&self, pressure: BroadcastPressure) {
		let changed = {
			let mut current = self.0.broadcast_pressure.write();
			if *current == pressure {
				false
			} else {
				*current = pressure;
		...
		if let Err(error) = self.try_send_actor_event(
			ActorEvent::BroadcastPressure { pressure },
			"broadcast_pressure",
		) {
			tracing::debug!(
				actor_id = %self.actor_id(),
				?error,
				"broadcast pressure changed before actor event delivery was available",
			);
		}
	}
	...
	pub fn broadcast(&self, name: &str, args: &[u8]) {
		let event = crate::actor::connection::OutgoingEvent::new(name, args);
		for connection in self.conns() {
			if connection.is_subscribed(name) {
				if let Err(error) = connection.try_send_event(event.clone()) {
					tracing::error!(
						?error,
