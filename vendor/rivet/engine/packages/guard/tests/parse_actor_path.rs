...
#[test]
fn parses_query_actor_get_or_create_paths_with_lane_hint() {
	let path = "/gateway/worker/socket?rvt-namespace=default&rvt-method=getOrCreate&rvt-runner=default&rvt-key=tenant&rvt-lane-hint=cpu-heavy";
...
fn rejects_input_for_get_queries() {
	...
	.to_string();
	assert!(err.contains(
		"query gateway method=get does not allow rvt-input, rvt-region, rvt-lane-hint, rvt-crash-policy, or rvt-pool params"
	));
}
...
#[test]
fn rejects_lane_hint_for_get_queries() {
	let err = parse_actor_path(
		"/gateway/lobby?rvt-namespace=default&rvt-method=get&rvt-lane-hint=cpu-heavy",
	)
	.unwrap_err()
	.to_string();
	assert!(err.contains(
