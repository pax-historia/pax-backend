...
func (r *RunnerConfigsServerlessMetadataError) UnmarshalJSON(data []byte) error {
	type unmarshaler RunnerConfigsServerlessMetadataError
	var value unmarshaler
	if err := json.Unmarshal(data, &value); err != nil {
		return err
	}
	*r = RunnerConfigsServerlessMetadataError(value)
	r._rawJSON = json.RawMessage(data)
	return nil
}

func (r *RunnerConfigsServerlessMetadataError) String() string {
	if len(r._rawJSON) > 0 {
		if value, err := core.StringifyJSON(r._rawJSON); err == nil {
			return value
		}
